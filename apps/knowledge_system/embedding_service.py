...
class EmbeddingService:
    ...
    def _ensure_model_loaded(self):
        ...
        if self.model is None:
            ...
            logger.info(
                f"Model loaded successfully. Embedding dimension: {self.vector_dimension}"
            )
    ...
    def generate_embeddings_batch(
        ...
        if texts_to_encode:
            ...
            for idx, text, embedding in zip(
                text_indices, texts_to_encode, new_embeddings
            ):
                embedding_list = embedding.tolist()
    ...
    def prepare_knowledge_text(self, knowledge_entry: Dict[str, Any]) -> str:
        ...
        # Add knowledge type as context (guard against None)
        if (
            "knowledge_type" in knowledge_entry
            and knowledge_entry["knowledge_type"] is not None
        ):
            parts.append(f"Type: {str(knowledge_entry['knowledge_type'])}")

        # Add main content (guard against None/non-str)
        if "content" in knowledge_entry:
            content_val = knowledge_entry["content"]
            if content_val is not None:
                parts.append(str(content_val))

        # Add tags if present
        if "tags" in knowledge_entry and knowledge_entry["tags"]:
            raw_tags = knowledge_entry["tags"]
            if isinstance(raw_tags, str):
                ...
                tags = raw_tags
            # Coerce to strings and filter out Nones
            safe_tags = [str(t) for t in (tags or []) if t is not None]
            if safe_tags:
        ...
        # Join only string parts; ignore any accidental non-strings
    ...
    def create_neo4j_vector_index_query(self) -> Dict[str, Any]:
        ...
        return {"query": query, "params": {"dimensions": self.vector_dimension}}

    def embed_knowledge_query(
