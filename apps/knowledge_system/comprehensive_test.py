...
def test_knowledge_retrieval(knowledge_id):
    ...
    result = KnowledgeQueryTool.execute(
        query_type="search", query_text="Comprehensive system test", limit=5
    )
    ...
    with driver.session() as session:
        ...
        if found:
            ...
            for record in found[:2]:
                # Handle both node return and projected fields
                try:
                    k = record["k"]
                    k_type = k.get("knowledge_type")
                    content = k.get("content") or ""
                except KeyError:
                    # Projected fields path
                    try:
                        k_type = (
                            record["type"]
                            if "type" in record.keys()
                            else record["knowledge_type"]
                        )
                    except KeyError:
                        k_type = "UNKNOWN"
                    try:
