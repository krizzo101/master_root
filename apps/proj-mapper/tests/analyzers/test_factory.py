...
from proj_mapper.claude_code_adapter import Analyzer, AnalyzerFactory
...
class TestAnalyzerFactory(unittest.TestCase):
    ...
    def test_get_analyzer_for_file(self):
        ...
        python_file = DiscoveredFile.create_mock(
            path="test.py", relative_path="test.py", file_type=FileType.PYTHON
        )
        ...
        text_file = DiscoveredFile.create_mock(
            path="test.txt", relative_path="test.txt", file_type=FileType.TEXT
        )
