...
def pipeline(config_manager):
    ...
    map_generator = MapGenerator(output_dir=Path(config_manager.get("output.directory")))
...
class TestPipelineIntegration:
    ...
    def test_pipeline_with_invalid_project(self, pipeline, test_project_dir):
        ...
        pipeline.config_manager.set("project.root", str(test_project_dir / "non_existent"))
    ...
    def test_analyzer_integration(self, pipeline, test_project_dir):
        ...
        if py_file and md_file:
            ...
            relationships = relationship_map.get_relationships_between(py_file[0], md_file[0])
            # There should be at least one relationship in one direction or the other
            relationships.extend(relationship_map.get_relationships_between(md_file[0], py_file[0]))
