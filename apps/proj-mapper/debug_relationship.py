...
try:
    ...
    from proj_mapper.cli.commands.relationship.discovery import _map_relationships, _build_relationship_graph
    from proj_mapper.analyzers.analyzer_factory import AnalyzerFactory
    ...
    def safe_to_dict(rel):
        return {
            ...
            "relationship_type": rel.relationship_type.name if hasattr(rel.relationship_type, "name") else str(rel.relationship_type),
    ...
    try:
        ...
        print(f"Found {len(code_elements)} code elements and {len(doc_elements)} doc elements")
