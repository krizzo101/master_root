...
from proj_mapper.analyzers.documentation.markdown.elements import detect_code_references, detect_markdown_links
...
class MarkdownAnalyzer(Analyzer):
    ...
    def can_analyze(self, file: DiscoveredFile) -> bool:
        ...
        """
        return file.file_type == FileType.MARKDOWN or file.get_extension() in self.supported_extensions
    ...
    def analyze(self, file: DiscoveredFile, content: Optional[str] = None) -> DocumentationAnalysisResult:
        ...
        try:
            ...
            logger.debug(f"Extracted {len(elements)} elements from {file.relative_path}")
