...
from proj_mapper.claude_code_adapter import MarkdownAnalyzer
...
class DocumentationAnalysisStage(PipelineStage):
    ...
    def process(
        self, context: PipelineContext, input_data: List[DiscoveredFile]
    ) -> List[Dict[str, Any]]:
        ...
        doc_files = [
            f
            for f in input_data
            if f.file_type == FileType.MARKDOWN or f.is_documentation()
        ]
        ...
        for file in doc_files:
            try:
                ...
                if analyzer:
                    ...
                    results.append(
                        {"file": file, "analysis": result, "success": result.success}
                    )
                else:
                    logger.warning(
                        f"No suitable analyzer found for: {file.relative_path}"
                    )
                    results.append(
                        {
                            "file": file,
        ...
        logger.info(
            f"Documentation analysis completed: {success_count}/{len(results)} files successful"
        )
