...
from proj_mapper.claude_code_adapter import AnalysisPipeline, AnalyzerFactory
...
class CodeAnalysisStage(PipelineStage):
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        """
        logger.debug(
            f"CodeAnalysisStage.process: Starting with context keys: {list(context.data.keys())}"
        )
        ...
        try:
            ...
            if not project:
                logger.error(
                    "CodeAnalysisStage.process: Project not available in context"
                )
                raise ValueError("Project not available in context")

            logger.info(f"Starting code analysis for project: {project.name}")
            logger.debug(
                f"CodeAnalysisStage.process: Project has {len(project.files)} total files"
            )
            ...
            logger.debug(
                f"CodeAnalysisStage.process: Filtered to {len(code_files)} code files"
            )
            ...
            code_project = DiscoveredProject(
                name=project.name, root_path=project.root_path, files=code_files
            )
            ...
            logger.debug(
                f"CodeAnalysisStage.process: Analysis pipeline completed with {len(results) if results else 0} results"
            )
            ...
            logger.debug(
                f"CodeAnalysisStage.process: Stored analysis results in context"
            )
            ...
            logger.debug(
                f"CodeAnalysisStage.process: Exception details: {traceback.format_exc()}"
            )
            raise
        finally:
            duration = time.time() - start_time
            logger.debug(
                f"CodeAnalysisStage.process: Completed in {duration:.2f} seconds"
            )
...
class DocumentationAnalysisStage(PipelineStage):
    ...
    def __init__(self, max_workers: int = 4):
        ...
        """
        logger.debug(
            f"Initializing DocumentationAnalysisStage with max_workers={max_workers}"
        )
        self.factory = AnalyzerFactory()
        self.pipeline = AnalysisPipeline(max_workers=max_workers)
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        """
        logger.debug(
            f"DocumentationAnalysisStage.process: Starting with context keys: {list(context.data.keys())}"
        )
        start_time = time.time()
        ...
        try:
            ...
            if not project:
                logger.error(
                    "DocumentationAnalysisStage.process: Project not available in context"
                )
                raise ValueError("Project not available in context")

            logger.info(f"Starting documentation analysis for project: {project.name}")
            logger.debug(
                f"DocumentationAnalysisStage.process: Project has {len(project.files)} total files"
            )
            ...
            logger.debug(
                f"DocumentationAnalysisStage.process: Filtered to {len(doc_files)} documentation files"
            )
            ...
            doc_project = DiscoveredProject(
                name=project.name, root_path=project.root_path, files=doc_files
            )

            # Analyze the documentation files
            logger.debug(
                f"DocumentationAnalysisStage.process: Starting documentation analysis pipeline"
            )
            results = self.pipeline.analyze_project(doc_project)
            logger.debug(
                f"DocumentationAnalysisStage.process: Analysis pipeline completed with {len(results) if results else 0} results"
            )

            # Get existing analysis results or create new ones
            analysis_results = context.get_data("analysis_results", {})
            logger.debug(
                f"DocumentationAnalysisStage.process: Retrieved existing analysis results with keys: {list(analysis_results.keys())}"
            )
            ...
            if "summary" in analysis_results:
                ...
                logger.debug(
                    "DocumentationAnalysisStage.process: Updated existing summary information"
                )
            else:
                analysis_results["summary"] = self.pipeline.generate_summary()
                logger.debug(
                    "DocumentationAnalysisStage.process: Created new summary information"
                )

            # Update context with the updated analysis results
            context.set_data("analysis_results", analysis_results)
            logger.debug(
                "DocumentationAnalysisStage.process: Updated analysis results in context"
            )

            logger.info(
                f"Completed documentation analysis: {len(results)} files analyzed"
            )

        except Exception as e:
            logger.error(
                f"DocumentationAnalysisStage.process: Error during documentation analysis: {e}"
            )
            logger.debug(
                f"DocumentationAnalysisStage.process: Exception details: {traceback.format_exc()}"
            )
            raise
        finally:
            duration = time.time() - start_time
            logger.debug(
                f"DocumentationAnalysisStage.process: Completed in {duration:.2f} seconds"
            )
...
class CombinedAnalysisStage(PipelineStage):
    ...
    def __init__(self, config: Optional[Union[int, Dict[str, Any]]] = None):
        ...
        if config is None:
            ...
            logger.debug(
                "CombinedAnalysisStage.__init__: Initialized with default config (None)"
            )
            ...
            logger.debug(
                f"CombinedAnalysisStage.__init__: Initialized with config dict: max_workers={self.max_workers}, "
            ...
            logger.debug(
                f"CombinedAnalysisStage.__init__: Initialized with max_workers={self.max_workers}"
            )
    ...
    def process(self, context: PipelineContext) -> PipelineContext:
        ...
        """
        logger.debug(
            f"CombinedAnalysisStage.process: Starting with context keys: {list(context.data.keys())}"
        )
        start_time = time.time()
        ...
        try:
            ...
            if not project:
                logger.error(
                    "CombinedAnalysisStage.process: Project not available in context"
                )
                raise ValueError("Project not available in context")
            ...
            logger.debug(
                f"CombinedAnalysisStage.process: Retrieved categorized files with types: {list(categorized_files.keys())}"
            )
            ...
            for file_type in code_file_types:
                if file_type in categorized_files:
                    logger.debug(
                        f"CombinedAnalysisStage.process: Adding {len(categorized_files.get(file_type, []))} {file_type} files for analysis"
                    )
            ...
            if code_files:
                ...
                logger.debug(
                    f"CombinedAnalysisStage.process: Code analysis completed with {len(code_results)} results"
                )
            ...
            if doc_files:
                ...
                logger.debug(
                    f"CombinedAnalysisStage.process: Documentation analysis completed with {len(doc_results)} results"
                )
                analysis_results.update(doc_results)
            else:
                logger.debug(
                    "CombinedAnalysisStage.process: No documentation files to analyze"
                )
            ...
            context.set_data(
                ...
                {
                    "code": {
                        str(path): results for path, results in code_results.items()
                    },
                    "doc": {
                        str(path): results for path, results in doc_results.items()
                    },
                },
            )
            logger.debug(
                "CombinedAnalysisStage.process: Updated analysis results in context"
            )
            ...
            logger.error(
                f"CombinedAnalysisStage.process: Error during combined analysis: {e}"
            )
            logger.debug(
                f"CombinedAnalysisStage.process: Exception details: {traceback.format_exc()}"
            )
            raise
        finally:
            duration = time.time() - start_time
            logger.debug(
                f"CombinedAnalysisStage.process: Completed in {duration:.2f} seconds"
            )
    ...
    def _analyze_files(self, files: List[DiscoveredFile]) -> Dict[str, Dict[str, Any]]:
        ...
        """
        logger.debug(
            f"CombinedAnalysisStage._analyze_files: Starting analysis of {len(files)} files with max_workers={self.max_workers}"
        )
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_file = {
                executor.submit(self._analyze_file, file): file for file in files
            }
            ...
            for future in as_completed(future_to_file):
                ...
                if completed_count % 10 == 0 or completed_count == len(files):
                    logger.debug(
                        f"CombinedAnalysisStage._analyze_files: Processed {completed_count}/{len(files)} files"
                    )
                ...
                try:
                    ...
                    if result:
                        ...
                        logger.debug(
                            f"CombinedAnalysisStage._analyze_files: Successfully analyzed {file.path}"
                        )
                    else:
                        logger.debug(
                            f"CombinedAnalysisStage._analyze_files: No results for {file.path}"
                        )
                except Exception as e:
                    logger.error(f"Error analyzing {file.path}: {e}")
                    logger.debug(
                        f"CombinedAnalysisStage._analyze_files: Exception details for {file.path}: {traceback.format_exc()}"
                    )

        duration = time.time() - start_time
        logger.debug(
            f"CombinedAnalysisStage._analyze_files: Completed analysis of {len(files)} files in {duration:.2f} seconds"
        )
        return results
    ...
    def _analyze_file(self, file: DiscoveredFile) -> Optional[Dict[str, Any]]:
        ...
        """
        logger.debug(
            f"CombinedAnalysisStage._analyze_file: Starting analysis of {file.path}"
        )
        start_time = time.time()
        ...
        try:
            ...
            if analyzer:
                logger.debug(
                    f"CombinedAnalysisStage._analyze_file: Using analyzer {analyzer.__class__.__name__} for {file.path}"
                )
                result = analyzer.analyze(file)
                duration = time.time() - start_time
                logger.debug(
                    f"CombinedAnalysisStage._analyze_file: Completed analysis of {file.path} in {duration:.2f} seconds"
                )
                return result
            else:
                logger.debug(
                    f"CombinedAnalysisStage._analyze_file: No suitable analyzer found for {file.path}"
                )
                return None
        except Exception as e:
            logger.error(f"Error analyzing {file.path}: {e}")
            logger.debug(
                f"CombinedAnalysisStage._analyze_file: Exception details: {traceback.format_exc()}"
            )
            duration = time.time() - start_time
            logger.debug(
                f"CombinedAnalysisStage._analyze_file: Failed analysis of {file.path} in {duration:.2f} seconds"
            )
            return None
...
def analyze_file(file_path: str, content: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        discovered_file = DiscoveredFile.create_mock(
            path=file_path_obj, relative_path=relative_path, file_type=file_type
        )
        ...
        try:
            logger.debug(
                f"analyze_file: Using analyzer {analyzer.__class__.__name__} for {file_path}"
            )
            result = analyzer.analyze(discovered_file, content)
            logger.debug(
                f"analyze_file: Analysis completed successfully for {file_path}"
            )

            duration = time.time() - start_time
            logger.debug(
                f"analyze_file: Completed analysis of {file_path} in {duration:.2f} seconds"
            )
            return result.dict()
        except Exception as e:
            logger.error(f"Error analyzing {file_path}: {e}")
            logger.debug(
                f"analyze_file: Exception during analysis: {traceback.format_exc()}"
            )
            return {"file_path": file_path, "success": False, "error_message": str(e)}
    except Exception as e:
...
def analyze_files(files: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for idx, file_info in enumerate(files):
        ...
        if not file_path:
            logger.debug(
                f"analyze_files: Skipping file at index {idx} due to missing path"
            )
            continue

        logger.debug(
            f"analyze_files: Processing file {idx+1}/{len(files)}: {file_path}"
        )
    ...
    duration = time.time() - start_time
    logger.debug(
        f"analyze_files: Completed analysis of {len(files)} files in {duration:.2f} seconds"
    )
    return results
...
def _determine_file_type(file_path: str) -> str:
    ...
    logger.debug(
        f"_determine_file_type: Determining file type for {file_path} with extension '{extension}'"
    )
    ...
    if extension in ("py", "pyi"):
        ...
        logger.debug(
            f"_determine_file_type: Unknown file type for extension '{extension}'"
        )
