...
from proj_mapper.analyzers.base import AnalysisResult, Analyzer
...
from proj_mapper.models.file import DiscoveredFile, FileType, DiscoveredProject
...
class AnalysisPipeline:
    ...
    def analyze_project(self, project: DiscoveredProject, 
                         file_types: Optional[Set[FileType]] = None) -> Dict[str, AnalysisResult]:
        ...
        logger.info(f"Analyzing {len(files_to_analyze)} files in project {project.name}")
        ...
        logger.info(f"Analysis complete. {len(self.results)} files analyzed successfully.")
        return self.results
    
    def _analyze_file(self, file: DiscoveredFile, analyzer: Analyzer) -> Optional[AnalysisResult]:
    ...
    def get_related_files(self, file_path: str, relationship_type: str = 'import',
                         recursive: bool = False, max_depth: int = 5) -> List[str]:
    ...
    def get_code_references_to_file(self, file_path: str) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for path, result in self.code_results.items():
            ...
            for element in result.elements:
                for ref in element.references:
                    ...
                    if (ref_id == file_path or 
                        ref_id == file_name or 
                        ref_id.endswith('/' + file_name) or
                        ref_id + '.py' == file_name or
                        Path(ref_id).name == file_name):
                        
                        file_refs.append({
                            'referencing_element': element.name,  # Use name instead of title for CodeElement
                            'location': element.location.to_dict() if element.location else None,
    ...
    def get_documentation_for_code(self, code_file_path: str) -> Dict[str, List[Dict[str, Any]]]:
            ...
            for element in result.elements:
                for ref in element.references:
                    if ref.reference_type == 'file' and (
                            ref.reference_id == file_name or 
                            ref.reference_id.endswith('/' + file_name)):
                        doc_refs.append({
                            'doc_element': element.title,
                            'element_type': element.element_type,
                            'location': element.location.to_dict() if element.location else None,
                            'context': element.content[:50] + '...' if len(element.content) > 50 else element.content
                        })
    ...
    def generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'total_code_elements': sum(len(r.elements) for r in self.code_results.values()),
            'total_doc_elements': sum(len(r.elements) for r in self.doc_results.values()),
            'file_types': {},
            'errors': []
        }
