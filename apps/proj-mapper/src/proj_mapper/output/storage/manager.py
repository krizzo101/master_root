...
class StorageManager:
    ...
    def _get_format_extension(self, format_type: str) -> str:
        ...
        """
        format_extensions = {"json": ".json", "yaml": ".yaml", "yml": ".yml"}
        return format_extensions.get(format_type.lower(), ".json")

    def store_map(
        self, project_map: ProjectMap, project_path: str, config: Configuration
    ) -> str:
        """Store a project map.
    ...
    def get_latest_map(self, project_path: str) -> str | None:
        """
        Return absolute path to the most recently modified map file
        for `project_path` using the active backend.
        ...
        return backend.get_latest_map(project_path)
