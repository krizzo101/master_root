...
class LocalFileSystemStorage(MapStorageProvider):
    ...
    def store_map(
        ...
        if isinstance(map_data, dict):
            ...
            map_data["metadata"]["format"] = format_extension[
                1:
            ]  # Remove the leading dot
            map_data["metadata"]["config"] = (
    ...
    def get_latest_map(self, project_path: str) -> Optional[str]:
        """
        Return path to the newest map file (by modification time) or None.
        """
        project_dir = Path(project_path) / self.base_dir
        if not project_dir.exists():
            return None

        # Gather candidate files
        candidates = list(project_dir.glob(f"*.*"))
        if not candidates:
            return None
    ...
    def _update_index(
        self, project_dir: str, filename: str, config: GeneratorConfig
    ) -> None:
