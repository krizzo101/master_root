...
from proj_mapper.models.analysis import (
    AnalysisResult,
    CodeAnalysisResult,
...
def detect_relationships(
    ...
    try:
        ...
        relationships = _map_relationships(
            code_analysis, doc_analysis, min_confidence=min_confidence
        )
        ...
        logger.info(
            f"Detected {len(relationships)} relationships with confidence >= {min_confidence}"
        )
        logger.info(f"Results saved to: {output_file}")
...
def _discover_files(
    ...
    file_discovery = FileDiscovery(
        include_patterns=all_include_patterns, exclude_patterns=exclude_set
    )
...
def _analyze_code(
    ...
    if not code_analyzers:
        logger.warning(
            f"No valid analyzers found for types: {analyzers}. Analysis will be empty."
        )
    ...
    for analyzer in code_analyzers:
        ...
        files = _discover_files(
            code_dir, file_filter, include_patterns, exclude_patterns
        )
        ...
        for file_path in files:
            try:
                ...
                logger.debug(
                    f"Analyzed {file_path} with {analyzer.__class__.__name__}, "
...
def _analyze_documentation(
    ...
    for analyzer in doc_analyzers:
        ...
        file_filter = analyzer.get_file_filter()
        files = _discover_files(
            docs_dir, file_filter, include_patterns, exclude_patterns
        )
        ...
        for file_path in files:
            try:
                ...
                if isinstance(file_elements, list):
                    ...
                    logger.warning(
                        f"Unexpected return type from {analyzer.__class__.__name__}.analyze_file: {type(file_elements)}"
                    )
...
def _map_relationships(
    ...
    if hasattr(code_analysis, "elements"):
        code_elements = [
            elem for elem in code_analysis.elements if isinstance(elem, CodeElement)
        ]
    ...
    if hasattr(doc_analysis, "elements"):
        doc_elements = [
            elem
            for elem in doc_analysis.elements
            if isinstance(elem, DocumentationElement)
        ]

    logger.info(
        f"Found {len(code_elements)} code elements and {len(doc_elements)} documentation elements"
    )
...
def _build_relationship_graph(relationships: List[Dict[str, Any]]) -> RelationshipGraph:
    ...
    for element_id in element_ids:
        ...
        graph.add_node(node_id=element_id, node_type=node_type, data={"id": element_id})
...
def _node_to_dict(node) -> Dict[str, Any]:
    ...
    return {"id": node.id, "node_type": node.node_type, "data": node.data}
