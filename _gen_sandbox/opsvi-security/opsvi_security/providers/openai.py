"""OpenAI provider for opsvi-security using Responses API (GPT-5 family).
...
from opsvi_security.providers.base import OpsviSecurityProvider
...
class OpenAIProvider(OpsviSecurityProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviSecurityError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviSecurityError(f"OpenAI generation failed: {last_err}")
