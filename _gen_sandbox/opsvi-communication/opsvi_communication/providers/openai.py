"""OpenAI provider for opsvi-communication using Responses API (GPT-5 family).
...
from opsvi_communication.providers.base import OpsviCommunicationProvider
...
class OpenAIProvider(OpsviCommunicationProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviCommunicationError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviCommunicationError(f"OpenAI generation failed: {last_err}")
