"""OpenAI provider for opsvi-fs using Responses API (GPT-5 family).
...
from opsvi_fs.providers.base import OpsviFsProvider
...
class OpenAIProvider(OpsviFsProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviFsError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviFsError(f"OpenAI generation failed: {last_err}")
