"""OpenAI provider for opsvi-data using Responses API (GPT-5 family).
...
from opsvi_data.providers.base import OpsviDataProvider
...
class OpenAIProvider(OpsviDataProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviDataError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviDataError(f"OpenAI generation failed: {last_err}")
