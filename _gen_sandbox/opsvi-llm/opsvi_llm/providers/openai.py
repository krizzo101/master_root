"""OpenAI provider for opsvi-llm using Responses API (GPT-5 family).
...
class OpenAIProvider(OpsviLlmProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviLlmError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviLlmError(f"OpenAI generation failed: {last_err}")
