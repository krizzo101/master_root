"""OpenAI provider for opsvi-http using Responses API (GPT-5 family).
...
from opsvi_http.providers.base import OpsviHttpProvider
...
class OpenAIProvider(OpsviHttpProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviHttpError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviHttpError(f"OpenAI generation failed: {last_err}")
