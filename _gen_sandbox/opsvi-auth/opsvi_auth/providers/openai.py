"""OpenAI provider for opsvi-auth using Responses API (GPT-5 family).
...
from opsvi_auth.providers.base import OpsviAuthProvider
...
class OpenAIProvider(OpsviAuthProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviAuthError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviAuthError(f"OpenAI generation failed: {last_err}")
