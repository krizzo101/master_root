"""OpenAI provider for opsvi-monitoring using Responses API (GPT-5 family).
...
from opsvi_monitoring.providers.base import OpsviMonitoringProvider
...
class OpenAIProvider(OpsviMonitoringProvider):
    ...
    async def generate_text(self, prompt: str, *, max_output_tokens: int = 800,
        ...
        if self._client is None:
            ...
            if not ok:
                raise OpsviMonitoringError("OpenAI client not initialized; set OPENAI_API_KEY")
        ...
        raise OpsviMonitoringError(f"OpenAI generation failed: {last_err}")
