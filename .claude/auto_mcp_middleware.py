...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/claude_auto_mcp.log'),
...
class AutoMCPMiddleware:
    ...
    def __init__(self, config_path: str = "/home/opsvi/master_root/.claude/config.json"):
    ...
    def analyze_prompt(self, prompt: str) -> Tuple[bool, Optional[str], Dict]:
        ...
        if instant_server:
            decision = (True, instant_server, {'reason': 'instant_trigger', 'triggers': triggers})
    ...
    def _check_instant_triggers(self, prompt_lower: str) -> Optional[str]:
        ...
        # V1 instant triggers
        v1_triggers = self.config['mcp_servers']['claude_code_v1']['auto_trigger_keywords']
        if any(trigger in prompt_lower for trigger in v1_triggers):
            return 'V1'
        
        # V2 instant triggers
        v2_triggers = self.config['mcp_servers']['claude_code_v2']['auto_trigger_keywords']
        if any(trigger in prompt_lower for trigger in v2_triggers):
            return 'V2'
        
        # V3 instant triggers
        v3_triggers = self.config['mcp_servers']['claude_code_v3']['auto_trigger_keywords']
    ...
    def get_v3_mode(self, prompt: str) -> str:
        ...
        prompt_lower = prompt.lower()
        mode_detection = self.config['mcp_servers']['claude_code_v3'].get('mode_detection', {})
