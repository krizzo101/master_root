...
class QdrantWrapper:
    ...
    async def upsert_chunks(self, embeddings: list[list[float]], payloads: list[dict[str, Any]]):
        points = [
    ...
    async def search(self, query_vec: list[float], limit: int = 10):
        res = await self.client.search(collection_name=CHUNK_COLLECTION, query_vector=query_vec, limit=limit)
        return res
