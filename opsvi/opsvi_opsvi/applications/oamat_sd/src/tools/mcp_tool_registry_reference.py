...
def _get_default_config():
    """Helper function to get default configuration for tool metadata"""
    try:
        from src.applications.oamat_sd.src.config.config_manager import ConfigManager

        return config
    except ImportError:
        # Fallback values if config not available (for testing)
...
class ToolMetadata:
    ...
    cost_per_call: float = field(
        default_factory=lambda: _get_default_config().tools.defaults["cost_per_call"]
    )
    avg_response_time: float = field(
        default_factory=lambda: _get_default_config().tools.defaults[
            "avg_response_time"
        ]
    )
    success_rate: float = field(
        default_factory=lambda: _get_default_config().tools.defaults["success_rate"]
    )
    last_health_check: Optional[datetime] = None
    health_status: str = field(
        default_factory=lambda: _get_default_config().tools.defaults["health_status"]
    )
    usage_count: int = field(
        default_factory=lambda: _get_default_config().tools.defaults["usage_count"]
    )
    error_count: int = field(
        default_factory=lambda: _get_default_config().tools.defaults["error_count"]
    )
...
class ToolExecutionResult:
    ...
    cost_incurred: float = field(
        default_factory=lambda: _get_default_config().tools.defaults["cost_incurred"]
    )
    tokens_used: int = field(
        default_factory=lambda: _get_default_config().tools.defaults["tokens_used"]
    )
...
class MCPToolRegistry:
    ...
    def __init__(
        ...
        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        print(
            f"[{timestamp}] 🔍 DEBUG: MCPToolRegistry basic setup complete, loading ConfigManager()..."
        )
        sys.stdout.flush()
    ...
    def _register_tool(
        ...
        **kwargs,
        ...
        # Get rate limits from config - NO FALLBACKS ALLOWED
        if "mcp_clients" not in self.config:
            raise RuntimeError(
                "mcp_clients configuration missing - NO FALLBACKS ALLOWED"
            )
        if name not in self.config["mcp_clients"]:
            raise RuntimeError(
                f"Client {name} not configured in mcp_clients - NO FALLBACKS ALLOWED"
            )
        if "rate_limits" not in self.config["mcp_clients"][name]:
            raise RuntimeError(
                f"Rate limits not configured for client {name} - NO FALLBACKS ALLOWED"
            )

        rate_limits = self.config["mcp_clients"][name]["rate_limits"]
    ...
    def _load_tool_chains(self):
        ...
        # Explicit configuration validation - NO FALLBACKS ALLOWED
        if "tool_chains" not in self.config:
            raise RuntimeError(
                "tool_chains configuration section missing - NO FALLBACKS ALLOWED"
            )

        tool_chains_config = self.config["tool_chains"]

        for chain_name, chain_config in tool_chains_config.items():
            if "tools" not in chain_config:
                raise RuntimeError(
                    f"Tools not configured for chain {chain_name} - NO FALLBACKS ALLOWED"
                )
    ...
    async def intelligent_tool_selection(
        self, query: str, task_type: str = "general", max_tools: Optional[int] = None
        ...
        """
        # Use config default if max_tools not specified - NO HARDCODED VALUES
        if max_tools is None:
            config = _get_default_config()
            max_tools = ConfigManager().tools.recommendations["max_tool_suggestions"]
            ...
            try:
                ...
                if instance:
                    ...
                    self.registered_tools[
                        tool_name
                    ].health_status = ConfigManager().status_values.health["healthy"]
                else:
                    health_status[tool_name] = False
                    self.registered_tools[
                        tool_name
                    ].health_status = ConfigManager().status_values.health["unhealthy"]
