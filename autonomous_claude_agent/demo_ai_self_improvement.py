...
class AIFirstSelfImprovingAgent:
    ...
    async def execute_task(self, task: str) -> dict:
        ...
        strategy_bonus = self.learned_strategies.get(
            strategy.decision, {}
        ).get('avg_performance', 0) * 0.1
    ...
    async def learn_from_execution(self, execution_result: dict):
        ...
        strategy_data['success_rate'] = (
            (1 - alpha) * strategy_data['success_rate'] + 
            alpha * (1.0 if execution_result['success'] else 0.0)
        )
        strategy_data['avg_performance'] = (
            (1 - alpha) * strategy_data['avg_performance'] + 
