...
class SelfImprovementDemo:
    ...
    async def run_demo(self):
        ...
        final_metrics = {
            ...
            'improvements_made': agent.improvements_made,
            'avg_iteration_time': sum(m['avg_iteration_time'] for m in self.metrics_history[-5:]) / min(5, len(self.metrics_history))
        }
    ...
    def calculate_improvement(self, initial, current):
        ...
        return {
            ...
            'performance': 1.0 if initial['avg_iteration_time'] == 0 else 
                          max(0.1, initial['avg_iteration_time']) / max(0.1, current['avg_iteration_time'])
        }
