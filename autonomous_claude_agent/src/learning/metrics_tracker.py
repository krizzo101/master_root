...
class MetricsTracker:
    ...
    async def gauge_set(self, name: str, value: float, tags: Optional[Dict[str, str]] = None):
        """Set a gauge metric"""
        await self.record(name, value, MetricType.GAUGE, tags)

    async def timer(self, name: str, duration: float, tags: Optional[Dict[str, str]] = None):
        """Record a timer metric"""
        await self.record(name, duration, MetricType.TIMER, tags)

    async def histogram(self, name: str, value: float, tags: Optional[Dict[str, str]] = None):
    ...
    async def _check_alerts(self, metric: Metric):
        ...
        if thresholds.get("rate_limit") is not None:
            recent = [
                m
                for m in self.metrics_buffer[metric.name]
    ...
    async def _trigger_alert(self, metric: Metric, alert_type: str, threshold: float):
        ...
        logger.warning(
            f"Alert triggered: {metric.name} {alert_type} "
            f"(value: {metric.value}, threshold: {threshold})"
        )

    async def get_metric_summary(
        self, name: str, period: Optional[timedelta] = None
    ...
    async def get_metrics_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "system": {"buffer_usage": {}, "metric_count": len(self.metrics_buffer)},
        }
    ...
    async def plot_metric(
        self, name: str, period: Optional[timedelta] = None, output_file: Optional[Path] = None
    ):
    ...
    async def export_metrics(self, output_file: Path, period: Optional[timedelta] = None):
        """Export metrics to JSON"""

        data = {"exported_at": datetime.now().isoformat(), "metrics": {}}
    ...
    async def cleanup_old_data(self, days: int = 7):
        ...
        with sqlite3.connect(self.db_path) as conn:
            # Clean metrics
            result = conn.execute("DELETE FROM metrics WHERE timestamp < ?", (cutoff.isoformat(),))
            metrics_deleted = result.rowcount

            # Clean aggregates
            result = conn.execute(
                "DELETE FROM aggregates WHERE period_end < ?", (cutoff.isoformat(),)
            )
            aggregates_deleted = result.rowcount

            # Clean resolved alerts
            result = conn.execute(
                "DELETE FROM alerts WHERE timestamp < ? AND resolved = 1", (cutoff.isoformat(),)
            )
