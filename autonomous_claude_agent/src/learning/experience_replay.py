...
class ExperienceReplay:
    ...
    def _save_experiences(self):
        ...
        experience_file = self.cache_dir / "experiences.pkl"
        try:
            data = {"experiences": list(self.buffer), "stats": self.stats}
    ...
    def _update_stats(self, experience: Experience):
        ...
        if experience.outcome == ExperienceOutcome.SUCCESS:
            self.stats["success_rate"] = (self.stats["success_rate"] * total + 1) / (total + 1)
        else:
            self.stats["success_rate"] = (self.stats["success_rate"] * total) / (total + 1)

        # Update average reward
        self.stats["avg_reward"] = (self.stats["avg_reward"] * total + experience.reward) / (
            total + 1
        )

        # Update average execution time
        self.stats["avg_execution_time"] = (
            self.stats["avg_execution_time"] * total + experience.execution_time
        ) / (total + 1)

    async def sample_batch(
        self, batch_size: Optional[int] = None, prioritized: bool = True
    ) -> List[Experience]:
        ...
        if prioritized and self.priority_buffer:
            ...
            priority_experiences = random.sample(
                [exp for _, exp in self.priority_buffer], priority_size
            )
    ...
    async def get_similar_experiences(
        self, task: str, context: Dict[str, Any], limit: int = 5
    ) -> List[Experience]:
    ...
    def _context_similarity(self, context1: Dict[str, Any], context2: Dict[str, Any]) -> float:
        ...
        if common_keys:
            value_matches = sum(1 for k in common_keys if str(context1[k]) == str(context2[k]))
    ...
    def _calculate_q_values(self, experiences: List[Experience]) -> Dict[str, float]:
        ...
        for exp in experiences:
            ...
            if key in q_values:
                q_values[key] = (
                    q_values[key] * (1 - self.learning_rate) + q_value * self.learning_rate
                )
            else:
    ...
    async def get_best_action(
        ...
        successful = [
            exp for exp in similar_experiences if exp.outcome == ExperienceOutcome.SUCCESS
        ]
    ...
    async def analyze_failures(self, task: Optional[str] = None) -> Dict[str, Any]:
        ...
        if task:
            failures = [
                exp
                for exp in self.by_task.get(task, [])
                if exp.outcome in [ExperienceOutcome.FAILURE, ExperienceOutcome.ERROR]
            ]
        else:
            failures = self.by_outcome.get(ExperienceOutcome.FAILURE, []) + self.by_outcome.get(
        ...
        async with self._lock:
            ...
            for i, exp1 in enumerate(self.buffer):
                ...
                for j, exp2 in enumerate(self.buffer[i + 1 :], i + 1):
                    ...
                    if (
                        exp1.task == exp2.task
                        and self._context_similarity(exp1.context, exp2.context) > min_similarity
                    ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        outcome_distribution = {
            outcome.value: len(experiences) for outcome, experiences in self.by_outcome.items()
        }
    ...
    async def export_experiences(
        self, output_file: Path, outcome_filter: Optional[ExperienceOutcome] = None
    ):
