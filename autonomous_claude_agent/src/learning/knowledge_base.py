...
class KnowledgeBase:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        if SentenceTransformer is not None:
            try:
                ...
                logger.warning(
                    "Sentence transformer model loading failed, semantic search disabled"
                )
                self.encoder = None
    ...
    async def search(
        self, query: str, limit: int = 10, knowledge_type: Optional[KnowledgeType] = None
    ) -> List[KnowledgeEntry]:
    ...
    async def merge_knowledge(self, entry_id1: str, entry_id2: str) -> Optional[str]:
        ...
        async with self._lock:
            ...
            merged = KnowledgeEntry(
                ...
                source=f"{entry1.source}, {entry2.source}"
                if entry1.source and entry2.source
                else None,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        by_type = {ktype.value: len(self.type_index[ktype]) for ktype in KnowledgeType}

        # Most used entries
        most_used = sorted(self.cache.values(), key=lambda e: e.usage_count, reverse=True)[:5]

        # Tag statistics
        tag_counts = {tag: len(entries) for tag, entries in self.tag_index.items()}
        ...
        return {
            ...
            "most_used": [
                {"id": e.id, "title": e.title, "type": e.type.value, "usage_count": e.usage_count}
                for e in most_used
            ],
    ...
    async def export_knowledge(
        self, output_file: Path, knowledge_type: Optional[KnowledgeType] = None
    ):
