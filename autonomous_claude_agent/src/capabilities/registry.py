...
class CapabilityRegistry:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        ...
        self._capability_index: Dict[str, Set[str]] = defaultdict(set)  # keyword -> capability names
        self._load_cache()
        ...
        if cache_file.exists():
            try:
                with open(cache_file, 'rb') as f:
                    ...
                    self.usage_stats = data.get('usage_stats', defaultdict(lambda: {
                        'count': 0,
                        'successes': 0,
                        'failures': 0,
                        'total_time': 0.0,
    ...
    async def register(self, capability: Capability, category: Optional[CapabilityCategory] = None) -> bool:
    ...
    def _auto_categorize(self, capability: Capability) -> Optional[CapabilityCategory]:
        ...
        # Check for patterns
        if any(word in name_lower + desc_lower for word in ['web', 'http', 'scrape', 'browse', 'url']):
            return CapabilityCategory.WEB_OPERATIONS
        elif any(word in name_lower + desc_lower for word in ['file', 'read', 'write', 'path', 'directory']):
            return CapabilityCategory.FILE_OPERATIONS
        elif any(word in name_lower + desc_lower for word in ['data', 'pandas', 'numpy', 'process', 'analyze']):
            return CapabilityCategory.DATA_PROCESSING
        elif any(word in name_lower + desc_lower for word in ['test', 'pytest', 'unittest', 'validate']):
        ...
        elif any(word in name_lower + desc_lower for word in ['ml', 'ai', 'neural', 'model', 'train']):
            return CapabilityCategory.MACHINE_LEARNING
        elif any(word in name_lower + desc_lower for word in ['system', 'process', 'cpu', 'memory']):
            return CapabilityCategory.SYSTEM_CONTROL
        elif any(word in name_lower + desc_lower for word in ['security', 'auth', 'encrypt', 'hash']):
            return CapabilityCategory.SECURITY
        elif any(word in name_lower + desc_lower for word in ['email', 'sms', 'notification', 'message']):
    ...
    async def record_usage(self, capability_name: str, success: bool, 
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        by_category = {
            cat.value: len(names) for cat, names in self.categories.items()
        }
