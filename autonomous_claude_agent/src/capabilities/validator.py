...
class CapabilityValidator:
    ...
    async def validate_capability(self, capability_name: str, 
    ...
    async def _validate_python_module(self, capability: Capability, 
                                     result: ValidationResult, level: ValidationLevel):
    ...
    async def _test_python_module_functions(self, module: Any, 
                                           result: ValidationResult, level: ValidationLevel):
            ...
            try:
                ...
                if not required_params:
                    ...
                    try:
                        ...
                        result.add_test_result(f'function_{name}', True, 
                                             metrics={'duration': duration})
                    except Exception as e:
                        # Function might need specific arguments
                        result.add_test_result(f'function_{name}', True,
                                             f"Requires arguments: {str(e)}")
                else:
    ...
    async def _validate_cli_tool(self, capability: Capability, 
                                result: ValidationResult, level: ValidationLevel):
    ...
    async def _test_cli_tool_execution(self, tool_name: str, 
                                      result: ValidationResult, level: ValidationLevel):
        ...
        if tool_name in safe_tests:
            try:
                ...
                if test_result.returncode == 0:
                    result.add_test_result('execution_test', True,
                                         metrics={'duration': duration})
                else:
                    result.add_test_result('execution_test', False,
                                         test_result.stderr)
                    
            except Exception as e:
                result.add_test_result('execution_test', False, str(e))
    
    async def _validate_mcp_tool(self, capability: Capability, 
                                result: ValidationResult, level: ValidationLevel):
        ...
        try:
            ...
            if server_name in mcp_config.get('servers', {}):
                ...
                if 'command' in server_config:
                    ...
                    result.add_test_result('has_command', False, "Server lacks command configuration")
                ...
                result.add_test_result('server_configured', False, f"Server {server_name} not in configuration")
                
        except Exception as e:
            result.add_test_result('mcp_validation', False, str(e))
    
    async def _validate_api_endpoint(self, capability: Capability, 
                                    result: ValidationResult, level: ValidationLevel):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(
                    ...
                    result.add_test_result('connectivity', response.status < 500,
                                         metrics={
    ...
    async def _test_api_performance(self, endpoint: str, api_config: Dict,
                                   result: ValidationResult, level: ValidationLevel):
        ...
        if response_times:
            ...
            result.add_test_result('performance_test', errors < num_requests * 0.1,
                                 metrics={
    ...
    async def _validate_system_capability(self, capability: Capability, 
                                         result: ValidationResult, level: ValidationLevel):
        ...
        try:
            ...
            result.add_test_result('memory_available', memory.available > 100 * 1024 * 1024,
                                 metrics={
            ...
            result.add_test_result('cpu_available', cpu_percent < 90,
                                 metrics={'cpu_percent': cpu_percent})
            
            # Disk check
            disk = psutil.disk_usage('/')
            result.add_test_result('disk_available', disk.free > 100 * 1024 * 1024,
                                 metrics={
    ...
    async def _validate_generic(self, capability: Capability, 
                               result: ValidationResult, level: ValidationLevel):
    ...
    async def _validate_compatibility(self, capability: Capability, result: ValidationResult):
        ...
        if dependencies:
            missing_deps = [
                ...
                if not self.registry.get_capability(d) or not self.registry.get_capability(d).available
            ]
    ...
    async def _validate_performance(self, capability: Capability, 
                                   result: ValidationResult, level: ValidationLevel):
    ...
    async def _update_registry_from_validation(self, capability: Capability, result: ValidationResult):
        ...
        if not result.passed and result.level != ValidationLevel.BASIC:
            ...
            logger.warning(f"Marking capability {capability.name} as unavailable due to validation failure")
    ...
    def _save_validation_result(self, result: ValidationResult):
        ...
        # Save to file
        result_file = self.validation_cache / f"{result.capability_name}_{result.timestamp.isoformat()}.json"
    ...
    async def validate_all(self, level: ValidationLevel = ValidationLevel.STANDARD) -> Dict[str, ValidationResult]:
