...
class CapabilityIntegrator:
    ...
    async def _integrate_npm_package(self, capability: Capability) -> bool:
        ...
        try:
            ...
            # Install globally
            result = subprocess.run(
                ["npm", "install", "-g", package_name], capture_output=True, text=True, timeout=300
            )
    ...
    def _create_rollback_point(
        self, capability: Capability, method: IntegrationMethod
    ) -> Dict[str, Any]:
    ...
    async def batch_integrate(
        self, capabilities: List[Capability], parallel: bool = True
        ...
        if parallel:
            ...
            tasks = [self.integrate_capability(cap) for cap in capabilities]
