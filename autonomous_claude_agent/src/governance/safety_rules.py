...
class SafetyRules:
    ...
    def add_rule(self, rule: SafetyRule) -> None:
        ...
        if self.audit_logger:
            self.audit_logger.log_event(
                event_type="SAFETY_RULE_ADDED", details={"rule_id": rule.rule_id, "name": rule.name}
            )
    ...
    def remove_rule(self, rule_id: str) -> None:
        ...
        if rule_id in self.rules:
            ...
            if self.audit_logger:
                self.audit_logger.log_event(
                    event_type="SAFETY_RULE_REMOVED", details={"rule_id": rule_id}
                )
    ...
    def _record_violation(self, violation: SafetyViolation) -> None:
        ...
        if self.audit_logger:
            self.audit_logger.log_event(
                ...
                severity="HIGH"
                if violation.severity in [SafetyLevel.HIGH, SafetyLevel.CRITICAL]
                else "MEDIUM",
    ...
    def _calculate_risk_score(
        self, severity: SafetyLevel, violations: List[SafetyViolation]
    ) -> float:
    ...
    def _get_recommendations(
        ...
        if severity == SafetyLevel.CRITICAL:
            recommendations.append(
                "This operation should not be executed without explicit admin approval"
            )
        ...
        for violation in violations:
            if violation.rule_id == "SYS001":
                recommendations.append(
                    "Use application-specific directories instead of system paths"
                )
    ...
    def export_violations_report(self, filepath: str) -> None:
        ...
        report = {
            ...
            "recent_violations": [v.to_dict() for v in self.violations_history[-100:]],
        }
