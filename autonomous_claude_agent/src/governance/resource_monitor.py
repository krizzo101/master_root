...
class ResourceMonitor:
    ...
    def start_monitoring(self) -> None:
        ...
        if self.audit_logger:
            self.audit_logger.log_event(
                event_type="RESOURCE_MONITORING_STARTED",
                details={"limits": self.limits.__dict__}
            )
    ...
    def track_tokens(self, count: int) -> bool:
        ...
        self.token_window = [
            (ts, c) for ts, c in self.token_window if ts > cutoff
        ]
    ...
    def track_api_call(self) -> bool:
        ...
        minute_window = [
            (ts, c) for ts, c in self.api_window if ts > minute_cutoff
        ]
        ...
        hour_window = [
            (ts, c) for ts, c in self.api_window if ts > hour_cutoff
        ]
    ...
    def get_resource_usage_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "throttle_states": {
                k.value: v for k, v in self._throttle_state.items()
            },
            "limits": {
