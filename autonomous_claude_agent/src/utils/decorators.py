...
def with_cache(
    cache_dir: Optional[Path] = None, ttl: Optional[int] = 3600, key_func: Optional[Callable] = None
):
    """
...
def with_logging(
    level: str = "INFO", log_args: bool = True, log_result: bool = False, log_time: bool = True
):
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            try:
                ...
                getattr(func_logger, level.lower())(f"{func.__name__} completed", **log_data)
            ...
            for param_name, validator in validators.items():
                if param_name in bound_args.arguments:
                    ...
                    if not validator(value):
                        raise ValueError(f"Invalid {param_name}: {value} failed validation")
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if check_func:
                if not await check_func(permission, *args, **kwargs):
                    raise PermissionError(f"Permission '{permission}' required for {func.__name__}")
            else:
                logger.warning(f"Permission check for '{permission}' not implemented")
...
if __name__ == "__main__":
    ...
    @validate_input(
        age=lambda x: x >= 0 and x <= 150, name=lambda x: isinstance(x, str) and len(x) > 0
    )
