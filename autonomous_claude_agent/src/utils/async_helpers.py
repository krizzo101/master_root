...
class AsyncTaskPool:
    ...
    async def submit(self, func: Callable, *args, task_id: Optional[str] = None, **kwargs) -> str:
        """
...
async def run_async_tasks(tasks: List[Callable], max_concurrent: Optional[int] = None) -> List[Any]:
    """
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if attempt < max_attempts - 1:
                logger.warning(
                    f"Attempt {attempt + 1} failed: {e}. " f"Retrying in {current_delay}s..."
                )
...
async def timeout_async(func: Callable, timeout: float, error_message: Optional[str] = None) -> Any:
    """
