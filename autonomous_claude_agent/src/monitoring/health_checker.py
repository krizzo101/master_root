...
class HealthStatus(Enum):
    ...
    def severity(self) -> int:
        ...
        severities = {self.HEALTHY: 0, self.DEGRADED: 1, self.UNKNOWN: 2, self.UNHEALTHY: 3}
...
class SystemHealth:
    ...
    def healthy_components(self) -> List[str]:
        ...
        return [
            name for name, comp in self.components.items() if comp.status == HealthStatus.HEALTHY
        ]
...
class DatabaseCheck(HealthCheck):
    ...
    def __init__(
        self, name: str, connection_func: Callable, query: str = "SELECT 1", critical: bool = True
    ):
        super().__init__(name, critical)
...
class FileSystemCheck(HealthCheck):
    ...
    def __init__(self, name: str, path: Path, writable: bool = False, critical: bool = False):
        super().__init__(name, critical)
        self.path = Path(path)
...
class HealthChecker:
    ...
    def _add_default_checks(self):
        ...
        # File system
        self.add_check(FileSystemCheck(name="data_directory", path=Path("./data"), writable=True))

        self.add_check(FileSystemCheck(name="cache_directory", path=Path("./cache"), writable=True))
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        # Add custom checks
        checker.add_check(
            HTTPCheck(name="api_endpoint", url="https://api.example.com/health", critical=True)
        )

        checker.add_check(ServiceCheck(name="redis", port=6379))
