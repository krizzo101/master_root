...
class ImprovementPattern:
    ...
    def update_metrics(self, success: bool):
        ...
        if success:
            ...
            self.success_rate = (
                (self.success_rate * (self.applications - 1)) / 
                self.applications
            )
...
class CodeGenerator:
    ...
    def generate_improvement(self, code: str, 
    ...
    def _find_matching_patterns(self, code: str, 
                               improvement_type: GenerationType) -> List[ImprovementPattern]:
    ...
    def _select_best_pattern(self, patterns: List[ImprovementPattern]) -> ImprovementPattern:
        ...
        return max(patterns, key=lambda p: (
            p.success_rate,
            -((datetime.now() - p.last_applied).days if p.last_applied else float('inf'))
        ))
    
    def _apply_pattern(self, code: str, pattern: ImprovementPattern,
    ...
    def _extract_variables(self, code: str, 
                          pattern: ImprovementPattern) -> Dict[str, Any]:
    ...
    def _select_template(self, improvement_type: GenerationType,
                        context: Dict[str, Any]) -> Optional[CodeTemplate]:
    ...
    def _apply_template(self, code: str, template: CodeTemplate,
                       context: Dict[str, Any]) -> GeneratedCode:
    ...
    def _generate_basic(self, code: str, improvement_type: GenerationType,
                       context: Dict[str, Any]) -> GeneratedCode:
        ...
        return GeneratedCode(
            code=improved_code,
            generation_type=improvement_type,
            metadata=context
        )
    ...
    def learn_pattern(self, original_code: str, improved_code: str,
                     improvement_type: GenerationType,
                     success: bool = True) -> ImprovementPattern:
        """Learn new pattern from successful improvement"""
        # Generate pattern ID
        pattern_id = hashlib.sha256(
            f"{original_code}{improved_code}".encode()
        ).hexdigest()[:16]
            ...
            if data['template_name'] in self.templates:
                pattern = ImprovementPattern(
                    ...
                    last_applied=datetime.fromisoformat(data['last_applied']) 
                                if data.get('last_applied') else None,
                    tags=data.get('tags', [])
                )
                self.patterns[pattern_id] = pattern
