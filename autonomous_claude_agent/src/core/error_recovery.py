...
class RetryStrategy(RecoveryStrategy):
    ...
    async def recover(self, context: ErrorContext) -> bool:
        ...
        logger.info(
            f"Retrying after {wait_time}s (attempt {context.retry_count + 1}/{context.max_retries})"
        )
...
class ErrorRecovery:
    ...
    async def attempt_recovery(self, error: Exception, context: Any) -> bool:
        ...
        try:
            ...
            error_context = ErrorContext(
                ...
                metadata={"traceback": traceback.format_exc(), "context": str(context)},
            )
    ...
    def _is_recurring_error(self, error_context: ErrorContext) -> bool:
        ...
        recent_errors = [
            e for e in self.error_history[-10:] if e.error_type == error_context.error_type
        ]
    ...
    def _record_successful_recovery(self, error_context: ErrorContext, strategy: RecoveryStrategy):
        ...
        if key not in self.successful_recoveries:
            self.successful_recoveries[key] = {"count": 0, "strategies": [], "last_success": None}

        self.successful_recoveries[key]["count"] += 1
        self.successful_recoveries[key]["last_success"] = datetime.now()

        logger.info(
            f"Recovery successful: {key} (total: {self.successful_recoveries[key]['count']})"
        )
    ...
    async def _find_known_solution(self, error_context: ErrorContext) -> Optional[Dict[str, Any]]:
        ...
        solutions = {
            "rate limit": {"action": "wait", "params": {"duration": 60}},
            "token limit": {"action": "compress_context", "params": {}},
            "memory": {"action": "free_memory", "params": {}},
        }
    ...
    async def learn_from_errors(self) -> Dict[str, Any]:
        ...
        for key, recoveries in self.successful_recoveries.items():
            ...
            if error_type in patterns:
                if (
                    patterns[error_type]["best_strategy"] is None
                    or recoveries["count"] > patterns[error_type]["recovery_success"]
                ):
