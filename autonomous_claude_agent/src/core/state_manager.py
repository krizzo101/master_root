...
class StateManager:
    ...
    async def save_checkpoint(self, state: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Save to file
            checkpoint_path = (
                self.checkpoint_dir / f"{checkpoint_id}.pkl{'z' if compressed else ''}"
            )
            ...
            await self.log_event(
                "checkpoint_failed", {"error": str(e)}, state.get("iteration", 0), False
            )
            raise
        ...
        try:
            ...
            async with self._get_db_connection() as conn:
                cursor = await conn.execute(
                    "SELECT * FROM state WHERE checkpoint_id = ?", (checkpoint_id,)
                )
            ...
            await self.log_event(
                "checkpoint_loaded",
                {"checkpoint_id": checkpoint_id},
                state_data["state"].get("iteration", 0),
                True,
            )
    ...
    async def list_checkpoints(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        async with self._get_db_connection() as conn:
            ...
            return [
                {"checkpoint_id": row[0], "timestamp": row[1], "iteration": row[2]} for row in rows
            ]
    ...
    async def get_metrics(self, metric_name: str, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        async with self._get_db_connection() as conn:
            ...
            return [{"timestamp": row[0], "iteration": row[1], "value": row[2]} for row in rows]
        ...
        try:
            ...
            if len(rows) > keep_count:
                ...
                for checkpoint_id, state_data in to_delete:
                    ...
                    async with self._get_db_connection() as conn:
                        await conn.execute(
                            "DELETE FROM state WHERE checkpoint_id = ?", (checkpoint_id,)
                        )
                        await conn.commit()
    ...
    async def export_state(self) -> Dict[str, Any]:
        ...
        # Export all metrics
        async with self._get_db_connection() as conn:
            cursor = await conn.execute("SELECT DISTINCT metric_name FROM metrics")
    ...
    async def import_state(self, state: Dict[str, Any]):
        ...
        for metric_name, values in state.get("metrics", {}).items():
            for value in values:
                await self.log_metric(metric_name, value["value"], value["iteration"])
