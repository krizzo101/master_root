...
class WebSearchEngine:
    ...
    def __init__(
        self, config: Optional[SearchConfig] = None, cache_manager: Optional[CacheManager] = None
    ):
        """
        ...
        self.rate_limiter = RateLimiter(self.config.rate_limit, per=60.0)
    ...
    async def initialize(self) -> None:
        ...
        if not self._session:
            ...
            connector = aiohttp.TCPConnector(limit=100, verify_ssl=self.config.verify_ssl)
    ...
    def _get_cache_key(self, query: str, **kwargs) -> str:
        ...
        key_parts = [str(self.config.provider.value), query, str(self.config.max_results)]
    ...
    async def _perform_search(self, query: str, **kwargs) -> List[SearchResult]:
        """
    ...
    async def _search_brave(self, query: str, **kwargs) -> List[SearchResult]:
        """Search using Brave API."""
        provider_config = self._providers[SearchProvider.BRAVE]

        params = {"q": query, "count": self.config.max_results, **kwargs}

        headers = provider_config["headers"].copy()

        try:
            async with self._session.get(
                provider_config["base_url"], params=params, headers=headers
            ) as response:
    ...
    async def _search_duckduckgo(self, query: str, **kwargs) -> List[SearchResult]:
        ...
        params = {"q": query, "format": "json", "no_html": "1", **kwargs}

        try:
            async with self._session.get(
                provider_config["base_url"], params=params, headers=provider_config["headers"]
            ) as response:
                response.raise_for_status()
                data = await response.json()
    ...
    async def batch_search(self, queries: List[str], **kwargs) -> Dict[str, List[SearchResult]]:
        """
    ...
    async def search_with_retry(self, query: str, **kwargs) -> List[SearchResult]:
        """
        ...
        for attempt in range(self.config.retry_count):
            try:
                ...
                if attempt < self.config.retry_count - 1:
                    ...
                    logger.warning(
                        f"Search attempt {attempt + 1} failed, " f"retrying in {delay}s: {e}"
                    )
