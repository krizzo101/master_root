...
def create_default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "context": {"max_tokens": 8000, "summarization_threshold": 6000, "compression_ratio": 0.3},
        "limits": {
            "memory_mb": 4096,
            "cpu_percent": 80,
            "disk_mb": 10240,
...
def display_agent_info(agent: AutonomousAgent, config: Dict[str, Any]):
    ...
    info_table.add_row(
        "Safety Mode", "Enabled" if config["safety"]["require_approval_for"] else "Disabled"
    )
...
async def async_main(
    ...
    try:
        ...
        if Path(config_path).exists():
            ...
            console.print(
                f"[yellow]Config file {config_path} not found. Creating default configuration.[/yellow]"
            )
        ...
        if dashboard and config["monitoring"]["dashboard_enabled"]:
            ...
            console.print(
                f"[green]Dashboard started at http://localhost:{config['monitoring']['dashboard_port']}[/green]"
            )
...
@click.command()
@click.option("--goal", required=True, help="The initial goal for the agent to work towards")
@click.option(
    "--config",
...
@click.option(
    "--dashboard/--no-dashboard", default=True, help="Enable web dashboard (default: enabled)"
)
@click.option("--checkpoint", help="Resume from checkpoint ID")
@click.option(
    "--max-iterations", default=1000, type=int, help="Maximum number of iterations (default: 1000)"
)
@click.option(
    "--log-level",
    default="INFO",
...
@click.option("--quiet", is_flag=True, help="Suppress banner and progress output")
def main(
    goal: str,
    config: str,
    mode: str,
