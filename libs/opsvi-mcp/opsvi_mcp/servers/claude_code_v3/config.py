...
class TimeoutConfig:
    ...
    def calculate_timeout(self, complexity: str, depth: int, file_count: int = 1) -> int:
...
class TaskDecompositionConfig:
    ...
    decomposition_patterns: Dict[str, list] = field(default_factory=lambda: {
        'server_creation': ['structure', 'config', 'models', 'server', 'tools', 'tests'],
...
class EnhancedConfig:
    ...
    decomposition: TaskDecompositionConfig = field(default_factory=TaskDecompositionConfig)
    ...
    def __post_init__(self):
        ...
        if os.getenv("CLAUDE_ADAPTIVE_CONCURRENCY"):
            self.recursion.adaptive_scaling = os.getenv("CLAUDE_ADAPTIVE_CONCURRENCY") == "true"
        ...
        if os.getenv("CLAUDE_ENABLE_ADAPTIVE_TIMEOUT"):
            self.timeout.enable_adaptive = os.getenv("CLAUDE_ENABLE_ADAPTIVE_TIMEOUT") == "true"
        ...
        if os.getenv("CLAUDE_ENABLE_TASK_DECOMPOSITION"):
            self.decomposition.enable_decomposition = os.getenv("CLAUDE_ENABLE_TASK_DECOMPOSITION") == "true"
        
        # Checkpointing
        if os.getenv("CLAUDE_ENABLE_CHECKPOINTING"):
            self.recursion.enable_checkpointing = os.getenv("CLAUDE_ENABLE_CHECKPOINTING") == "true"
        
        # Recovery
        if os.getenv("CLAUDE_ENABLE_RECOVERY"):
            self.recovery.enable_recovery = os.getenv("CLAUDE_ENABLE_RECOVERY") == "true"
        
        # Learning
        if os.getenv("CLAUDE_ENABLE_LEARNING"):
            self.timeout.learning_enabled = os.getenv("CLAUDE_ENABLE_LEARNING") == "true"
