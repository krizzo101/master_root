...
class CriticAgent:
    ...
    def _calculate_overall_score(self, result: ReviewResult) -> float:
        ...
        weighted_score = sum(
            scores.get(aspect, 0) * weight for aspect, weight in weights.items()
        )
    ...
    def _calculate_code_metrics(self, code: str, language: str) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "average_line_length": sum(len(l) for l in lines) / len(lines)
            if lines
            else 0,
        }
    ...
    async def _review_configuration(self, content: str) -> ReviewResult:
        ...
        if any(
            pattern in content for pattern in ["localhost", "127.0.0.1", "password"]
        ):
            result.issues.append(
                Issue(
                    severity=IssueSeverity.MEDIUM,
