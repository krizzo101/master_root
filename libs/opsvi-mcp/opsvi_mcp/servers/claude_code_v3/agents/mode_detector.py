...
class ModeDetector:
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'review': re.compile(
                r'\b(review|critique|evaluate|assess|audit|inspect)\b',
                re.IGNORECASE
            ),
            'test': re.compile(
                r'\b(test|validate|verify|check|ensure|confirm)\b',
                re.IGNORECASE
            ),
            'document': re.compile(
            ...
            'fix': re.compile(
                r'\b(fix|debug|resolve|repair|solve|troubleshoot)\b',
                re.IGNORECASE
            ),
            'improve': re.compile(
    ...
    def _analyze_complexity(self, task: str) -> str:
        ...
        complex_keywords = [
            'implement', 'create', 'develop', 'build', 'integrate',
            'multiple', 'comprehensive', 'complete', 'full'
        ]
        very_complex_keywords = [
            'server', 'framework', 'architecture', 'system', 
    ...
    def _check_production_indicators(self, task: str, context: Optional[Dict]) -> bool:
        ...
        prod_indicators = [
            'production', 'prod', 'live', 'deploy',
            'release', 'customer', 'client'
        ]
