...
class TaskDecomposer:
    ...
    def _compile_patterns(self) -> Dict[TaskType, re.Pattern]:
        ...
        return {
            ...
            TaskType.MULTI_FILE_OPERATION: re.compile(
                r'(multiple|all|several|batch).*(file|component|module)', 
                re.IGNORECASE
            ),
            TaskType.REFACTORING: re.compile(
                r'(refactor|restructure|reorganize|optimize|improve)', 
                re.IGNORECASE
            ),
            TaskType.TESTING: re.compile(
                r'(test|testing|coverage|unittest|integration test)', 
                re.IGNORECASE
            ),
            TaskType.DOCUMENTATION: re.compile(
                r'(document|documentation|readme|api doc|comment)', 
                re.IGNORECASE
            )
        }
    ...
    def estimate_complexity(self, task: str) -> str:
        ...
        very_complex_keywords = ['server', 'framework', 'architecture', 'system', 'infrastructure']
    ...
    def _decompose_server_creation(self, task: str, context: Dict) -> List[SubTask]:
        ...
        subtasks.append(SubTask(
            ...
            can_parallel=False,
            dependencies=[f"{server_name}_structure", f"{server_name}_config", f"{server_name}_models"],
        ...
        for key, group_tasks in groups.items():
            if len(group_tasks) > 2:
                ...
                consolidated.append(SubTask(
                    ...
                    metadata={'consolidated': True, 'original_count': len(group_tasks)}
                ))
            else:
