...
class TimeoutManager:
    ...
    def calculate_timeout(
        ...
        base_timeout = self.config.timeout.calculate_timeout(
            complexity, depth, file_count
        )
    ...
    def _calculate_from_history(
        self, task_type: str, complexity: str, depth: int, file_count: int
    ) -> Optional[int]:
        ...
        successful_times = [t.actual_time_ms for t in similar_tasks if t.completed]
        ...
        percentile_90 = (
            statistics.quantiles(successful_times, n=10)[8]
            if len(successful_times) > 10
            else max(successful_times)
        )
    ...
    def get_timeout_with_decay(
        self, initial_timeout: int, elapsed_ms: int, checkpoint_progress: float = 0.0
    ) -> int:
    ...
    def should_extend_timeout(
        self, job_id: str, elapsed_ms: int, progress: float, recent_activity: bool
    ) -> Tuple[bool, int]:
    ...
    def calculate_retry_timeout(
        self, original_timeout: int, failure_reason: str, attempt_number: int
    ) -> int:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "type_stats": {
                k: {"count": len(v), "avg_time": statistics.mean(v) if v else 0}
