...
class TestingConfig:
    ...
    coverage_enabled: bool = (
        os.environ.get("COVERAGE_ENABLED", "true").lower() == "true"
    )
    ...
    load_test_duration: int = int(
        os.environ.get("LOAD_TEST_DURATION", "300")
    )  # seconds
    load_test_think_time: float = float(os.environ.get("LOAD_TEST_THINK_TIME", "1.0"))

    # Test Types Configuration
    test_types: Dict[str, bool] = field(
        default_factory=lambda: {
            "unit": os.environ.get("ENABLE_UNIT_TESTS", "true").lower() == "true",
            "integration": os.environ.get("ENABLE_INTEGRATION_TESTS", "true").lower()
            == "true",
            "e2e": os.environ.get("ENABLE_E2E_TESTS", "true").lower() == "true",
            "mutation": os.environ.get("ENABLE_MUTATION_TESTS", "false").lower()
            == "true",
            "performance": os.environ.get("ENABLE_PERF_TESTS", "true").lower()
            == "true",
    ...
    mutation_timeout: int = int(
        os.environ.get("MUTATION_TIMEOUT", "10")
    ...
    include_edge_cases: bool = (
        os.environ.get("INCLUDE_EDGE_CASES", "true").lower() == "true"
    )
    include_negative_tests: bool = (
        os.environ.get("INCLUDE_NEGATIVE_TESTS", "true").lower() == "true"
    )
    ...
    report_include_logs: bool = (
        os.environ.get("REPORT_INCLUDE_LOGS", "true").lower() == "true"
    )
    report_include_screenshots: bool = (
        os.environ.get("REPORT_INCLUDE_SCREENSHOTS", "false").lower() == "true"
    )

    # Execution Configuration
    parallel_execution: bool = (
        os.environ.get("PARALLEL_EXECUTION", "true").lower() == "true"
    )
    ...
    cache_test_results: bool = (
        os.environ.get("CACHE_TEST_RESULTS", "true").lower() == "true"
    )
    ...
    analyze_flaky_tests: bool = (
        os.environ.get("ANALYZE_FLAKY_TESTS", "true").lower() == "true"
    )
    flaky_test_threshold: int = int(os.environ.get("FLAKY_TEST_THRESHOLD", "3"))
    analyze_test_dependencies: bool = (
        os.environ.get("ANALYZE_DEPENDENCIES", "true").lower() == "true"
    )

    # Integration Configuration
    ci_integration: str = os.environ.get(
