...
from .models import (
    ScanType, SeverityLevel, SecurityIssue, ScanRequest, ScanResult,
    DependencyAuditResult, DependencyVulnerability, SecretScanResult,
    SecretFinding, LicenseCheckResult, LicenseInfo, BestPracticesResult,
    BestPracticeViolation, VulnerabilityFix, SecurityReport
)
...
class SecurityServer:
    ...
    async def _run_command(self, cmd: List[str], cwd: Optional[Path] = None) -> tuple[int, str, str]:
            ...
            if stdout:
                ...
                for issue in result.get("results", []):
                    ...
                    issues.append(SecurityIssue(
                        ...
                        severity=SeverityLevel(severity) if severity in ["low", "medium", "high", "critical"] else SeverityLevel.LOW,
            ...
            if stdout:
                ...
                for finding in result.get("results", []):
                    ...
                    issues.append(SecurityIssue(
                        ...
                        type=finding.get("extra", {}).get("message", "Security Issue"),
                        severity=SeverityLevel(severity) if severity in ["low", "medium", "high", "critical"] else SeverityLevel.LOW,
    ...
    async def detect_secrets(self, project_path: Path) -> SecretScanResult:
        ...
        try:
            for file_path in project_path.rglob("*"):
                if file_path.is_file() and file_path.stat().st_size < self.config.max_file_size:
...
async def security_scan(
    ...
    scan_type: str = Field(default="all", description="Type of scan: sast/dependencies/secrets/all"),
    severity_threshold: str = Field(default="low", description="Minimum severity: low/medium/high/critical")
) -> Dict:
...
async def license_check(
    project_path: str = Field(description="Path to project"),
    allowed_licenses: Optional[List[str]] = Field(default=None, description="List of allowed licenses")
) -> Dict:
    """Check license compliance"""
...
async def generate_security_report(
    project_path: str = Field(description="Path to project"),
    output_format: str = Field(default="json", description="Report format: json/html/markdown")
) -> Dict:
    ...
    report = SecurityReport(
        report_id=hashlib.md5(f"{project_path}{datetime.now()}".encode()).hexdigest()[:8],
        project_name=path.name,
        project_path=project_path,
        scan_date=datetime.now(),
        scanner_version="1.0.0",
        sast_result=ScanResult(**scan_result) if "error" not in scan_result else None,
        dependency_audit=DependencyAuditResult(**dep_audit) if "error" not in dep_audit else None,
        secret_scan=SecretScanResult(**secret_scan) if "error" not in secret_scan else None,
        license_check=LicenseCheckResult(**license_result) if "error" not in license_result else None,
        best_practices=BestPracticesResult(**best_practices) if "error" not in best_practices else None,
    ...
    if report.critical_issues > 0:
        report.immediate_actions.append("Fix critical security vulnerabilities immediately")
    ...
    # Save report
    report_file = security_server.config.report_directory / f"security_report_{report.report_id}.{output_format}"
