...
class Metric(BaseModel):
    ...
    value: Optional[float] = Field(
        default=None, description="Current value (for gauges)"
    )
    count: Optional[int] = Field(default=None, description="Count (for counters)")
    sum: Optional[float] = Field(
        default=None, description="Sum (for histograms/summaries)"
    )
    buckets: Optional[Dict[float, int]] = Field(
        default=None, description="Histogram buckets"
    )
    quantiles: Optional[Dict[float, float]] = Field(
        default=None, description="Summary quantiles"
    )
...
class LogEntry(BaseModel):
    ...
    attributes: Dict[str, Any] = Field(
        default_factory=dict, description="Log attributes"
    )
...
class Dashboard(BaseModel):
    ...
    variables: List[Dict[str, Any]] = Field(
        default_factory=list, description="Dashboard variables"
    )
...
class PerformanceSnapshot(BaseModel):
    ...
    latency_p50_ms: Optional[float] = Field(
        default=None, description="P50 latency in ms"
    )
    latency_p95_ms: Optional[float] = Field(
        default=None, description="P95 latency in ms"
    )
    latency_p99_ms: Optional[float] = Field(
        default=None, description="P99 latency in ms"
    )
...
class CollectMetricsRequest(BaseModel):
    ...
    metric_names: Optional[List[str]] = Field(
        default=None, description="Specific metrics to collect (None for all)"
    )
...
class StartTraceRequest(BaseModel):
    ...
    attributes: Dict[str, Any] = Field(
        default_factory=dict, description="Initial attributes"
    )
    parent_trace_id: Optional[str] = Field(
        default=None, description="Parent trace ID for distributed tracing"
    )
...
class CreateDashboardRequest(BaseModel):
    ...
    time_range: Optional[Dict[str, str]] = Field(
        default=None, description="Default time range"
    )
...
class LogAggregationRequest(BaseModel):
    ...
    start_time: Optional[datetime] = Field(
        default=None, description="Start time for logs"
    )
    end_time: Optional[datetime] = Field(default=None, description="End time for logs")
...
class CustomMetricDefinition(BaseModel):
    ...
    description: str = Field(description="Metric description")
    unit: Optional[str] = Field(default=None, description="Metric unit")
    labels: List[str] = Field(default_factory=list, description="Required labels")
    expression: Optional[str] = Field(
        default=None, description="Metric calculation expression"
    )
