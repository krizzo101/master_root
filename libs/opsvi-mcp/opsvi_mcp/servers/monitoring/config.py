...
class OpenTelemetryConfig(BaseModel):
    ...
    service_name: str = Field(
        default="monitoring-mcp", description="Service name for traces"
    )
    endpoint: str = Field(
        default="http://localhost:4317", description="OTLP endpoint for traces"
    )
    protocol: str = Field(default="grpc", description="Protocol for OTLP (grpc/http)")
    headers: Dict[str, str] = Field(
        default_factory=dict, description="Additional headers for OTLP requests"
    )
...
class AlertConfig(BaseModel):
    ...
    slack_channels: List[str] = Field(
        default_factory=list, description="Slack channels for alerts"
    )
    default_severity: str = Field(
        default="warning", description="Default alert severity"
    )
    throttle_minutes: int = Field(
        default=15, description="Alert throttling period in minutes"
    )
    rules_file: str = Field(
        default="alert_rules.yaml", description="Path to alert rules configuration"
    )
...
class LoggingConfig(BaseModel):
    ...
    output_dir: str = Field(
        default="/tmp/monitoring_logs", description="Directory for log files"
    )
    max_file_size: str = Field(default="100MB", description="Maximum log file size")
    max_files: int = Field(
        default=10, description="Maximum number of log files to keep"
    )
...
class DashboardConfig(BaseModel):
    ...
    grafana_url: Optional[str] = Field(
        default="http://localhost:3000", description="Grafana URL for dashboards"
    )
    grafana_api_key: Optional[str] = Field(default=None, description="Grafana API key")
    auto_provision: bool = Field(default=True, description="Auto-provision dashboards")
    dashboard_dir: str = Field(
        default="dashboards", description="Directory containing dashboard definitions"
    )
    refresh_interval: str = Field(
        default="5s", description="Default dashboard refresh interval"
    )
...
class PerformanceConfig(BaseModel):
    ...
    latency_threshold_ms: int = Field(
        default=1000, description="Latency threshold in milliseconds"
    )
    profile_enabled: bool = Field(
        default=False, description="Enable performance profiling"
    )
    profile_dir: str = Field(
        default="/tmp/profiles", description="Directory for performance profiles"
    )
...
class CustomMetricsConfig(BaseModel):
    ...
    definitions_file: str = Field(
        default="custom_metrics.yaml", description="Path to custom metrics definitions"
    )
    export_interval: int = Field(default=60, description="Export interval in seconds")
    aggregation_window: int = Field(
        default=300, description="Aggregation window in seconds"
    )
...
class MonitoringServerConfig(BaseModel):
    ...
    server_name: str = Field(default="monitoring-mcp", description="Server name")
    host: str = Field(default="localhost", description="Server host")
    port: int = Field(default=5005, description="Server port")

    prometheus: PrometheusConfig = Field(
        default_factory=PrometheusConfig, description="Prometheus configuration"
    )
    opentelemetry: OpenTelemetryConfig = Field(
        default_factory=OpenTelemetryConfig, description="OpenTelemetry configuration"
    )
    ...
    custom_metrics: CustomMetricsConfig = Field(
        default_factory=CustomMetricsConfig, description="Custom metrics configuration"
    )

    data_dir: str = Field(
        default="/tmp/monitoring_data", description="Directory for monitoring data"
    )
    config_reload_interval: int = Field(
        default=60, description="Configuration reload interval in seconds"
    )
    ...
    def from_env(cls) -> "MonitoringServerConfig":
        ...
        if prom_enabled := os.getenv("PROMETHEUS_ENABLED"):
            config_dict.setdefault("prometheus", {})["enabled"] = (
                prom_enabled.lower() == "true"
            )
        ...
        if otel_enabled := os.getenv("OTEL_ENABLED"):
            config_dict.setdefault("opentelemetry", {})["enabled"] = (
                otel_enabled.lower() == "true"
            )
