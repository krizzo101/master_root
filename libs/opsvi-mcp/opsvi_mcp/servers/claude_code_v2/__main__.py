    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.StreamHandler(
                sys.stderr
            ),  # Use stderr to avoid stdout JSON conflicts
...
def check_environment():
    ...
    logger.debug(
        f"  Results Directory: {os.environ.get('CLAUDE_RESULTS_DIR', '/tmp/claude_results')}"
    )
    logger.debug(
        f"  Max Concurrent L1: {os.environ.get('CLAUDE_MAX_CONCURRENT_L1', '10')}"
    )
    logger.debug(f"  Max Recursion: {os.environ.get('CLAUDE_MAX_RECURSION', '3')}")
    logger.debug(
        f"  Default Timeout: {os.environ.get('CLAUDE_DEFAULT_TIMEOUT', '600')}s"
    )

    if not os.environ.get("CLAUDE_CODE_TOKEN"):
        logger.warning(
            "CLAUDE_CODE_TOKEN not set - Server will run but Claude API calls will fail"
        )
    else:
