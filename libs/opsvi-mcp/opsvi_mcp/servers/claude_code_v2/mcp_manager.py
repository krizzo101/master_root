...
class MCPRequirementAnalyzer:
    ...
    MCP_PATTERNS = {
        "git": [
            ...
            r"\brebase\b", r"\bcheckout\b", r"\bclone\b", r"\bpull\b", r"\bpush\b"
        ],
        "firecrawl": [
            r"\bscrape\b", r"\bcrawl\b", r"\bweb\s+page\b", r"\bwebsite\b",
            r"\bextract\s+from\s+url\b", r"\bfetch\s+url\b"
        ],
        "time": [
            r"\bcurrent\s+time\b", r"\btimestamp\b", r"\bdate\b", 
            r"\btimezone\b", r"\bconvert\s+time\b"
        ],
        "shell_exec": [
            r"\brun\s+command\b", r"\bexecute\b", r"\bshell\b", r"\bbash\b",
            r"\bterminal\b", r"\bcommand\s+line\b"
        ],
        "research_papers": [
            r"\barxiv\b", r"\bpaper\b", r"\bresearch\b", r"\bacademic\b",
            r"\bjournal\b", r"\bpublication\b"
        ],
        "db": [
            r"\bdatabase\b", r"\bneo4j\b", r"\bcypher\b", r"\bquery\b",
            r"\bSQL\b", r"\btable\b", r"\bschema\b"
        ],
        "tech_docs": [
            r"\bdocumentation\b", r"\blibrary\b", r"\bAPI\s+docs\b",
            r"\bframework\b", r"\bpackage\s+docs\b"
        ],
        "mcp_web_search": [
            r"\bsearch\s+web\b", r"\bgoogle\b", r"\bbrave\s+search\b",
            r"\binternet\b", r"\bonline\b"
        ],
        "calc": [
            r"\bcalculate\b", r"\bmath\b", r"\barithmetic\b", r"\bcompute\b",
            r"\bsum\b", r"\baverage\b", r"\bequation\b"
        ],
        "claude-code-wrapper": [
            r"\bdebug\b", r"\bfix\s+error\b", r"\binvestigate\b",
            r"\banalyze\s+code\b", r"\bwhy\b"
        ],
        "claude-code-v2": [
            r"\bparallel\b", r"\ball\s+files\b", r"\bevery\b", r"\bmultiple\b",
            r"\bbatch\b", r"\bconcurrent\b"
        ],
        "claude-code-v3": [
            r"\bproduction\b", r"\benterprise\b", r"\brobust\b",
            r"\bcomprehensive\b", r"\bquality\b"
        ]
    }
...
class MCPAvailabilityChecker:
    ...
    async def wait_for_mcp_servers(
        self, 
        required_servers: Optional[Set[str]] = None,
...
class MCPConfigManager:
    ...
    CONFIGURATIONS = {
        ...
        "git_only": {
            "description": "Only Git operations",
            "servers": ["git"]
        },
        "web_focused": {
                    ...
                    for pattern in cls.MCP_INIT_PATTERNS:
                        ...
                        if match:
                            ...
                            if server_name not in server_timestamps:
                                ...
                                logger.debug(f"Detected MCP server '{server_name}' initialized")
...
class IntelligentMCPManager:
    ...
    async def prepare_instance(
        ...
        instance_id: str,
        task: str,
        force_config_type: Optional[str] = None
    ) -> MCPInstanceConfig:
        """
    ...
    def _select_config_type(self, required_servers: Set[str]) -> str:
        ...
        for config_name, config_info in MCPConfigManager.CONFIGURATIONS.items():
            if config_info["servers"] and set(config_info["servers"]) >= required_servers:
                return config_name
    ...
    def _record_metrics(self, instance_id: str, metrics: Dict[str, MCPServerMetrics]):
        ...
        # Log summary
        successful = sum(1 for m in metrics.values() if m.status == MCPServerStatus.READY)
        failed = sum(1 for m in metrics.values() if m.status == MCPServerStatus.FAILED)
        avg_init_time = sum(
            m.initialization_duration_ms for m in metrics.values() 
    ...
    def get_performance_summary(self) -> Dict:
        ...
        summary = {
            ...
            "overall_avg_init_ms": sum(all_init_times) / len(all_init_times) if all_init_times else 0,
