...
- Authentication and authorization""",
    ...
    def _generate_task_prompt(cls, task: str) -> str:
        ...
        if any(word in task_lower for word in ["git", "commit", "merge", "branch"]):
            prompts.append(
                "Follow Git best practices: atomic commits, clear messages, branch hygiene."
            )
        ...
        if any(
            ...
            prompts.append(
                "Measure performance before and after changes. Provide metrics."
            )

        # Documentation tasks
        if any(word in task_lower for word in ["document", "readme", "docstring"]):
            prompts.append(
                "Write clear, example-driven documentation suitable for the target audience."
            )

        # Security tasks
        if any(
            word in task_lower
            for word in ["security", "vulnerability", "auth", "encryption"]
        ):
            prompts.append(
                "Apply security best practices and consider OWASP guidelines."
            )
    ...
    def _generate_context_prompt(cls, inherited_context: Dict[str, Any]) -> str:
        ...
        if "previous_errors" in inherited_context:
            prompts.append(
                f"Previous attempts encountered these errors: {inherited_context['previous_errors']}. Avoid these issues."
            )

        if "constraints" in inherited_context:
            prompts.append(
                f"Work within these constraints: {inherited_context['constraints']}"
            )
        ...
        if "style_guide" in inherited_context:
            prompts.append(
                f"Follow this style guide: {inherited_context['style_guide']}"
            )
...
class AgentSpecializer:
    ...
    def determine_role(task: str) -> str:
        ...
        if any(
            word in task_lower for word in ["analyze", "review", "examine", "inspect"]
        ):
            return "analyzer"
        elif any(
            word in task_lower for word in ["debug", "fix", "error", "bug", "issue"]
        ):
            return "debugger"
        elif any(
            word in task_lower
            ...
            word in task_lower for word in ["test", "pytest", "unittest", "coverage"]
        ):
            return "tester"
        elif any(
            word in task_lower for word in ["review", "audit", "quality", "standards"]
        ):
            return "reviewer"
        elif any(
            word in task_lower
            for word in ["design", "architect", "structure", "pattern"]
        ):
            return "architect"
        elif any(
            word in task_lower
            for word in ["security", "vulnerability", "auth", "encrypt"]
        ):
            return "security"
        else:
            return "general"
...
def prepare_specialized_agent(
    ...
    context = context_mgr.build_child_context(
        job_id=job_id, task=task, parent_id=parent_id, role=role
    )

    # Create or inherit session
    if inherit_session and context.parent_session_id:
        # Can't reuse same session ID, must use --resume
        context.session_id = session_mgr.create_session(
            job_id, context.parent_session_id
        )
    ...
    return {
        ...
        "command_args": {
            ...
            "--resume": context.parent_session_id
            if inherit_session and context.parent_session_id
            else None,
        },
    }
