...
class UnifiedMCPOrchestrator:
    ...
    def _init_servers(self):
        ...
        if self.config["servers"]["claude_code"]["enabled"]:
            try:
                ...
                claude_config = ClaudeConfig(**self.config["servers"]["claude_code"]["config"])
        ...
        if self.config["servers"]["openai_codex"]["enabled"]:
            try:
                ...
                codex_config = CodexConfig(**self.config["servers"]["openai_codex"]["config"])
        ...
        if self.config["servers"]["cursor_agent"]["enabled"]:
            try:
                ...
                cursor_config = CursorConfig(**self.config["servers"]["cursor_agent"]["config"])
    ...
    def _setup_orchestration_tools(self):
        ...
        async def analyze_and_implement(
            ...
            if visualize and "cursor_agent" in self.servers:
                diagram_data = results.get("analysis", {}).get("architecture", requirements)
        ...
        async def code_review_pipeline(
            ...
            if fix_issues and "openai_codex" in self.servers:
                issues = results.get("review", {}).get("issues", "No specific issues found")
    ...
    async def _invoke_codex(self, prompt: str, mode: str = "generate", **kwargs) -> Dict[str, Any]:
