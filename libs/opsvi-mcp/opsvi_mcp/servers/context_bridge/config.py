...
class ContextBridgeConfig(BaseSettings):
    ...
    context_ttl_seconds: int = Field(
        default=3600,
        description="TTL for cached context in seconds (Redis only)"
    )
    ...
    memory_pubsub_max_queue_size: int = Field(
        default=1000,
        description="Maximum queue size per channel for in-memory pub/sub"
    )
    memory_pubsub_cleanup_interval: int = Field(
        default=300,
        description="Cleanup interval for inactive subscriptions (seconds)"
    )
