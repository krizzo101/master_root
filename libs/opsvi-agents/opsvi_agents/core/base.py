...
class BaseAgent(ABC):
    ...
    def _attempt_recovery(self, error: Exception, prompt: str, kwargs: Dict) -> Optional[AgentResult]:
        ...
        if self.context.checkpoints:
            ...
            self._logger.info("Recovering from checkpoint", checkpoint=last_checkpoint["name"])
