...
class AgentMemory:
    ...
    def recall(self, query: str, memory_type: str = "all") -> List[Dict[str, Any]]:
        ...
        if memory_type in ["all", "short_term"]:
            results.extend([m for m in self.short_term if query.lower() in str(m).lower()])
        ...
        if memory_type in ["all", "episodic"]:
            results.extend([m for m in self.episodic if query.lower() in str(m).lower()])
...
"""
    ...
    async def think(self, prompt: str, context: Optional[str] = None) -> str:
        ...
        if context:
            messages.append(Message(role="system", content=f"Additional context:\n{context}"))
        ...
        try:
            ...
            if hasattr(response, 'usage'):
                self.context.token_usage["prompt"] += response.usage.get("prompt_tokens", 0)
                self.context.token_usage["completion"] += response.usage.get("completion_tokens", 0)
                self.context.token_usage["total"] += response.usage.get("total_tokens", 0)
    ...
    async def plan(self, goal: str, constraints: List[str] = None) -> List[Dict[str, Any]]:
        """
...
"""
        ...
        self.context.observations.append(f"Observation: {observation}\nReflection: {reflection}")
        return reflection
        ...
        if event in self._callbacks:
            for callback in self._callbacks[event]:
                try:
                    ...
                    self._logger.error(f"Callback failed for event {event}", error=str(e))
    ...
    async def _attempt_recovery(self, error: Exception, prompt: str, kwargs: Dict) -> Optional[AgentResult]:
...
"""
        ...
        try:
            ...
            alternative_prompt = f"Given the error, try an alternative approach:\n{recovery_thought}"
