...
class TelemetryTracker:
    ...
    def _load_metrics(self) -> None:
        ...
        if self.metrics_path.exists():
            try:
                ...
                self._logger.info(f"Loaded metrics for {len(self.aggregated)} operations")
    ...
    def get_statistics(self, operation: str = None) -> Dict[str, Any]:
        ...
        if operation:
            ...
            avg_duration = agg["total_duration"] / agg["count"] if agg["count"] > 0 else 0
            success_rate = agg["success_count"] / agg["count"] if agg["count"] > 0 else 0
