...
- Consider trade-offs and alternatives""",
    ...
    async def _generate_with_llm(self, request: CodeGenerationRequest) -> CodeGenerationResult:
        """Generate code using the LLM."""
        # Get appropriate system prompt
        system_prompt = self.system_prompts.get(
            request.generation_type,
            self.system_prompts[CodeGenerationType.FUNCTION]
        )
...
\nProvide the response in JSON format with the following structure:
{
    ...
    def _parse_llm_response(self, response: Any, request: CodeGenerationRequest) -> CodeGenerationResult:
        ...
        try:
            ...
            logger.warning(f"Failed to parse structured response, using raw content: {e}")
    ...
    async def implement_pattern(
        ...
        **kwargs
        ...
        description = f"Implement the {pattern_name} design pattern in {language} for: {context}"
