...
class ExecutorAgent(BaseAgent):
    ...
    def _execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "task_executed", command=command, success=True, duration=duration
            )
        ...
        for i, sub_task in enumerate(tasks):
            ...
            if "execution_result" in result:
                ...
                if not exec_result.success and stop_on_error:
                    logger.warning(
                        f"Batch execution stopped at task {i+1} due to error"
                    )
                    break
            else:
                # Error in execution
    ...
    def _execute_tool(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"tool_result": result, "tool": tool_name}
    ...
    def _execute_api(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"api_result": result, "endpoint": endpoint, "method": method}
        ...
        self.tools.update(
            {
                ...
                "format": {
                    "function": lambda template, **kwargs: template.format(**kwargs),
    ...
    def _call_api(
        ...
        return {
            ...
            "endpoint": endpoint,
            "method": method,
            "response": {"message": "Mock API response", "params": params},
        }
    ...
    def _run_generic(
        self, command: str, params: Dict[str, Any], context: Dict[str, Any]
    ) -> Any:
        """Run generic command."""
    ...
    def _track_execution(self, result: ExecutionResult):
        ...
        self.execution_history.append(
            {"timestamp": time.time(), "result": result.to_dict()}
        )
    ...
    def _get_history(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if filter_success is not None:
            history = [h for h in history if h["result"]["success"] == filter_success]

        return {"history": history, "total_executions": len(self.execution_history)}

    def shutdown(self) -> bool:
        """Shutdown the executor agent."""
        logger.info(
            "executor_agent_shutdown", total_executions=len(self.execution_history)
        )
