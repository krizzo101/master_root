...
class Critique:
    ...
    def _update_score(self):
        ...
        total_deduction = sum(
            deductions.get(issue.level, 0) / issue.priority
...
class CriticAgent(BaseAgent):
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        super().__init__(config or AgentConfig(
            ...
            capabilities=["review", "critique", "suggest_improvements", "quality_assurance"],
            max_retries=2,
            timeout=60
        ))
        self.critiques = {}
    ...
    def critique(self, 
                ...
                standards: Optional[Dict[str, Any]] = None) -> Critique:
    ...
    def _perform_critique(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not dimensions:
            ...
            dimensions = [QualityDimension[d.upper()] if isinstance(d, str) else d 
                         for d in dimensions]
        ...
        if critique.dimension_scores:
            critique.overall_score = sum(critique.dimension_scores.values()) / len(critique.dimension_scores)
        else:
    ...
    def _infer_dimensions(self, work: Any) -> List[QualityDimension]:
        ...
        if isinstance(work, str):
            ...
            if any(indicator in work.lower() for indicator in ["def ", "class ", "function", "import"]):
                dimensions.extend([
    ...
    def _evaluate_dimension(self, work: Any, dimension: QualityDimension, 
                          standards: Dict[str, Any]) -> Tuple[float, List[Issue]]:
    ...
    def _identify_strengths(self, work: Any, dimension_scores: Dict[QualityDimension, float]) -> List[str]:
    ...
    def _generate_improvements(self, issues: List[Issue]) -> List[str]:
        ...
        for dimension, dim_issues in dimension_issues.items():
            if len(dim_issues) > 2:
                improvements.append(f"Focus on improving {dimension.value} across the work")
    ...
    def _suggest_improvements(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        suggestions = {
            ...
            "estimated_score_improvement": min(100, critique.overall_score + len(actionable) * 5)
        }
    ...
    def _check_rule(self, work: Any, rule_name: str, rule_value: Any) -> Tuple[bool, str]:
