...
class AnalysisAgent(BaseAgent):
    ...
    def analyze(self, 
                ...
                depth: InsightLevel = InsightLevel.MODERATE) -> AnalysisReport:
    ...
    def _perform_analysis(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "actionable_insights": [i.to_dict() for i in report.get_actionable_insights()]
        }
    ...
    def _compute_statistical_metrics(self, data: Any) -> List[Metric]:
        ...
        if isinstance(data, list) and data and all(isinstance(x, (int, float)) for x in data):
            ...
            metrics.append(Metric("median", statistics.median(data), category="central"))
            metrics.append(Metric("min", min(data), category="range"))
            metrics.append(Metric("max", max(data), category="range"))
            
            if len(data) > 1:
                metrics.append(Metric("stdev", statistics.stdev(data), category="dispersion"))
                metrics.append(Metric("variance", statistics.variance(data), category="dispersion"))
            ...
            if len(data) >= 4:
                ...
                metrics.append(Metric("iqr", sorted_data[q3_idx] - sorted_data[q1_idx], category="quartiles"))
        
        elif isinstance(data, dict):
            # Dictionary statistics
            metrics.append(Metric("keys_count", len(data), category="basic"))
            metrics.append(Metric("total_size", len(str(data)), unit="bytes", category="size"))
            ...
            metrics.append(Metric("size", len(str(data)), unit="bytes", category="size"))
    ...
    def _analyze_structure(self, data: Any) -> List[Metric]:
        ...
        if isinstance(data, (list, tuple)):
            ...
            metrics.append(Metric("depth", self._calculate_depth(data), category="structure"))
            metrics.append(Metric("homogeneous", self._is_homogeneous(data), category="structure"))
            
        elif isinstance(data, dict):
            metrics.append(Metric("type", "mapping", category="structure"))
            metrics.append(Metric("keys", len(data), category="structure"))
            metrics.append(Metric("depth", self._calculate_depth(data), category="structure"))
            metrics.append(Metric("nested_dicts", self._count_nested_dicts(data), category="structure"))
            
        elif isinstance(data, set):
            metrics.append(Metric("type", "set", category="structure"))
            metrics.append(Metric("size", len(data), category="structure"))
            metrics.append(Metric("unique_types", len(set(type(x).__name__ for x in data)), category="structure"))
    ...
    def _analyze_behavior(self, data: Any) -> List[Metric]:
        ...
        # Analyze mutability
        metrics.append(Metric("mutable", isinstance(data, (list, dict, set)), category="behavior"))
    ...
    def _analyze_performance(self, data: Any) -> List[Metric]:
        ...
        metrics.append(Metric("memory_usage", size_bytes, unit="bytes", category="performance"))
        ...
        if isinstance(data, list):
            ...
            metrics.append(Metric("contains_complexity", "O(1)", category="performance"))
    ...
    def _create_insights(self, metrics: List[Metric], patterns: List[Pattern], 
                        depth: InsightLevel) -> List[Insight]:
            ...
            if metric.name == "stdev" and metric.value is not None:
                if metric.value < 1:
                    ...
                    insights.append(Insight(
                        ...
                        confidence=0.85,
                        actionable=True,
                        recommendations=["Consider if low variance is expected", 
        ...
        for pattern in patterns:
            if pattern.type == "repetition" and pattern.occurrences > 5:
                ...
                insights.append(Insight(
                    ...
                    confidence=0.9,
                    actionable=True,
                    recommendations=["Consider data deduplication",
    ...
    def _generate_summary(self, metrics: List[Metric], patterns: List[Pattern], 
    ...
    def _behavioral_analysis(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "metrics": [m.to_dict() for m in metrics],
            "behavior_profile": "dynamic" if isinstance(data, (list, dict, set)) else "static"
        }
    ...
    def _estimate_memory_usage(self, data: Any) -> int:
        ...
        if isinstance(data, (int, float)):
            ...
            return sum(self._estimate_memory_usage(item) for item in data) + 8 * len(data)
        elif isinstance(data, dict):
            return sum(self._estimate_memory_usage(k) + self._estimate_memory_usage(v) 
                      for k, v in data.items()) + 16 * len(data)
        else:
            return len(str(data))
    ...
    def shutdown(self) -> bool:
        ...
        logger.info("analysis_agent_shutdown", 
