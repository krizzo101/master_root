...
class ValidationRule:
    ...
    def _validate_type(self, data: Any) -> Optional['ValidationIssue']:
        ...
        if not isinstance(data, expected_type):
            return ValidationIssue(
                ...
                message=self.message or f"Expected type {expected_type.__name__}, got {type(data).__name__}"
            )
        return None
    ...
    def _validate_pattern(self, data: Any) -> Optional['ValidationIssue']:
        ...
        pattern = self.condition if isinstance(self.condition, str) else str(self.condition)
        ...
        if isinstance(self.condition, dict):
            ...
            if data < min_val or data > max_val:
                return ValidationIssue(
                    ...
                    value=data,
                    severity=self.severity,
                    message=self.message or f"Value {data} outside range [{min_val}, {max_val}]"
                )
        return None
...
class ValidatorAgent(BaseAgent):
    ...
    def _register_builtin_rules(self):
        ...
        # Type rules
        self._add_rule("required", ValidationType.TYPE, lambda x: x is not None, 
                      Severity.ERROR, "Field is required")
        self._add_rule("string", ValidationType.TYPE, str,
                      Severity.ERROR, "Must be a string")
        self._add_rule("integer", ValidationType.TYPE, int,
                      Severity.ERROR, "Must be an integer")
        self._add_rule("float", ValidationType.TYPE, float,
                      Severity.ERROR, "Must be a float")
        self._add_rule("boolean", ValidationType.TYPE, bool,
                      Severity.ERROR, "Must be a boolean")
        self._add_rule("list", ValidationType.TYPE, list,
                      Severity.ERROR, "Must be a list")
        self._add_rule("dict", ValidationType.TYPE, dict,
                      Severity.ERROR, "Must be a dictionary")
        
        # Format rules
        self._add_rule("email", ValidationType.PATTERN, 
                      r'^[\w\.-]+@[\w\.-]+\.\w+$',
                      Severity.ERROR, "Must be a valid email")
        self._add_rule("url", ValidationType.PATTERN,
                      r'^https?://[\w\.-]+[\w\.-]+[\w\.-]*$',
                      Severity.ERROR, "Must be a valid URL")
        self._add_rule("phone", ValidationType.PATTERN,
                      r'^\+?[\d\s\-\(\)]+$',
                      Severity.ERROR, "Must be a valid phone number")
        self._add_rule("date", ValidationType.PATTERN,
                      r'^\d{4}-\d{2}-\d{2}$',
                      Severity.ERROR, "Must be YYYY-MM-DD format")
        
        # Range rules
        self._add_rule("positive", ValidationType.RANGE,
                      {"min": 0},
                      Severity.ERROR, "Must be positive")
        self._add_rule("percentage", ValidationType.RANGE,
                      {"min": 0, "max": 100},
                      Severity.ERROR, "Must be between 0 and 100")
    
    def _register_builtin_schemas(self):
        """Register built-in validation schemas."""
        # User schema
        user_rules = [
            self.rules.get("required"),
            self.rules.get("email")
        ]
        ...
        api_rules = [
            self.rules.get("required"),
            self.rules.get("string")
        ]
        api_rules = [r for r in api_rules if r]
        
        if api_rules:
            self._create_schema_internal("api_request", "API request validation", api_rules)
    
    def _add_rule(self, name: str, type: ValidationType, condition: Any,
                 severity: Severity, message: str) -> ValidationRule:
    ...
    def _create_schema_internal(self, name: str, description: str, 
                               rules: List[ValidationRule]) -> ValidationSchema:
    ...
    def _check_completeness(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if required_fields:
            present_count = len(required_fields) - len(missing_fields) - len(empty_fields)
    ...
    def _audit_data(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        data = task.get("data")
        audit_types = task.get("audit_types", ["type", "format", "consistency", "completeness"])
        ...
        if "type" in audit_types:
            type_result = self._validate_data({
                "data": data,
                "rules": ["required"]
            })
            audit_results["checks_performed"].append({
                "check": "type",
                "passed": type_result.get("valid", False)
            })
        ...
        if "completeness" in audit_types and isinstance(data, dict):
            completeness_result = self._check_completeness({
                "data": data,
                "required_fields": list(data.keys())  # Check all fields
            })
            audit_results["checks_performed"].append({
                "check": "completeness",
        ...
        passed_checks = sum(1 for check in audit_results["checks_performed"] 
                          if check.get("passed", False))
        total_checks = len(audit_results["checks_performed"])
        audit_results["audit_score"] = passed_checks / total_checks if total_checks > 0 else 0
        
        return {"audit_results": audit_results}
    
    def shutdown(self) -> bool:
        """Shutdown the validator agent."""
        logger.info("validator_agent_shutdown", 
