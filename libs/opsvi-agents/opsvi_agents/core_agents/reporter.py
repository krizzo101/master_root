...
class ReporterAgent(BaseAgent):
    ...
    def generate_report(self,
                       ...
                       format: ReportFormat = ReportFormat.MARKDOWN) -> Report:
        """Generate a report."""
        result = self.execute({
            "action": "report",
            "title": title,
    ...
    def _create_technical_sections(self, data: Any) -> List[Section]:
        ...
        for section_def in template["sections"]:
            ...
            # Extract content from data
            if isinstance(data, dict):
                content = data.get(section_def["key"], self._generate_technical_content(data, section_def["key"]))
            else:
    ...
    def _format_value(self, value: Any) -> Any:
        ...
        if isinstance(value, (dict, list)):
            return json.dumps(value, indent=2) if len(str(value)) < 1000 else str(value)[:1000] + "..."
    ...
    def _apply_format(self, content: Any, format_type: str) -> str:
        ...
        if format_type == "json":
            return json.dumps(content, indent=2) if not isinstance(content, str) else content
    ...
    def shutdown(self) -> bool:
        ...
        logger.info("reporter_agent_shutdown", 
