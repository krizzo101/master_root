...
class ParallelOrchestrationPattern:
    ...
    def create_parallel_sends(
        target_node: str, tasks: List[Dict[str, Any]], state_key: str = "task_data"
    ) -> List[Send]:
        """
    ...
    def build_parallel_graph(
        ...
        if routing_function:
            ...
            workflow.add_conditional_edges("coordinator", default_routing)
...
class StateGraphBuilder:
    ...
    def add_parallel_coordinator(
        self, name: str, task_extractor: Callable, target_node: str
    ) -> "StateGraphBuilder":
        """
    ...
    def build(self) -> StateGraph:
        ...
        logger.info(
            f"Built StateGraph with {len(self.nodes)} nodes, {len(self.edges)} edges"
        )
...
class SendAPIPattern:
    ...
    def create_batch_sends(
        ...
        logger.info(
            f"Created {len(batches)} batches with max {batch_size} parallel sends"
        )
        return batches
