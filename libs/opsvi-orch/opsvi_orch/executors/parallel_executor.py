...
from ..patterns.langgraph_patterns import (
    ...
    async def execute_parallel_tasks(
        ...
        try:
            ...
            graph = self._build_execution_graph(
                task_executor, result_aggregator, state_type or dict
            )
            ...
            if debug:
                ...
                logger.info(
                    f"Tasks completed: {result.tasks_completed}, failed: {result.tasks_failed}"
                )
        ...
        def aggregator_node(state):
            ...
            if result_aggregator:
                ...
                if "task_error" in state:
                    errors.append(
                        {"index": state.get("task_index"), "error": state["task_error"]}
                    )
    ...
    async def execute_with_retry(
        ...
        for attempt in range(max_retries):
            result = await self.execute_parallel_tasks(
                tasks, task_executor, debug=attempt > 0  # Debug on retries
            )
            ...
            logger.info(
                f"Retrying parallel execution (attempt {attempt + 2}/{max_retries})"
            )
