...
class V1OrchestrationIntegration:
    ...
    def __init__(
        ...
        self.job_manager = JobManager(
            config=JobConfig(
                max_concurrent=10, enable_recursion=True, enable_checkpointing=True
            )
        )
        ...
        self.batch_executor = ClaudeBatchExecutor(
            job_executor=self.claude_executor, max_concurrent=10
        )
    ...
    def create_mcp_tools(self) -> Dict[str, Callable]:
            ...
            try:
                if batchId:
                    results = await self.job_manager.collect_batch_results(
                        batch_id=batchId, wait=wait, timeout_ms=timeout * 1000
                    )
                elif jobIds:
                    # Create ad-hoc batch for specific jobs
                    results = {"jobIds": jobIds, "results": {}, "errors": {}}
                    ...
                    return json.dumps(
                        {"error": "Must provide either batchId or jobIds"}, indent=2
                    )
        ...
        async def claude_spawn_recursive(
            ...
            try:
                ...
                from ..executors.recursive_executor import (
                    RecursiveClaudeExecutor,
                    RecursionConfig,
                )

                config = RecursionConfig(
                    max_depth=maxDepth, max_children_per_parent=childrenPerLevel
                )
...
def enhance_v1_with_orchestration(
    v1_server_module: Any, claude_token: Optional[str] = None
) -> None:
    """
