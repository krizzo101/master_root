...
class ResultProcessor:
    ...
    async def process_task_result(
        ...
        try:
            ...
            await self._process_lineage(
                task_id=task_id, result=result, project_id=project_id, run_id=run_id
            )
    ...
    async def process_pipeline_results(
        ...
        for task_id, result in results.items():
            ...
            if result.get("status") == "ok":
                ...
                summary["total_execution_time"] += result.get(
        ...
        for agent_type, stats in agent_stats.items():
            if stats["total_tasks"] > 0:
                summary["agent_performance"][agent_type] = {
                    ...
                    "average_score": stats["total_score"] / stats["successful_tasks"]
    ...
    async def _update_task_status(
        self, task_id: str, status: str, result_id: str
    ) -> None:
        ...
        await self.neo4j_client.execute_query(
            update_query, {"task_id": task_id, "status": status, "result_id": result_id}
        )

    async def _process_lineage(
        self, task_id: str, result: Dict[str, Any], project_id: str, run_id: str
    ) -> None:
        ...
        await self.neo4j_client.execute_query(
            lineage_query,
            {"task_id": task_id, "project_id": project_id, "run_id": run_id},
        )
