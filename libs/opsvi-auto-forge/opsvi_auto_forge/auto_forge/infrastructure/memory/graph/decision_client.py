...
class DecisionClient:
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if result.single():
                    ...
                    logger.info(
                        f"Persisted decision record for task {record.decision.task_id}"
                    )
                    return True
                else:
                    logger.error(
                        f"Failed to persist decision record for task {record.decision.task_id}"
                    )
                    return False
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if result.single():
                    ...
                    if success:
                        decision_success.labels(
                            strategy="unknown", model="unknown"
                        ).inc()
                    else:
                        decision_failure.labels(
                            strategy="unknown", model="unknown"
                    ...
                    logger.error(
                        f"Failed to update decision execution for task {task_id}"
                    )
                    return False
    ...
    def add_escalation_event(
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if result.single():
                    logger.info(
                        f"Recorded escalation for task {task_id}: {from_model} -> {to_model}"
                    )
                    return True
                else:
    ...
    def get_decision_performance_stats(
        self, time_period: str = "7d", limit: int = 100
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                return {
                    ...
                    "success_rate": sum(r["successful_decisions"] for r in records)
                    / max(sum(r["total_decisions"] for r in records), 1),
                    "strategies": [
                        {
                            "strategy": r["strategy"],
                            "model": r["model"],
                            ...
                            "success_rate": r["successful_decisions"]
                            / max(r["total_decisions"], 1),
                            "avg_cost": r["avg_cost"],
    ...
    def persist_decision_evidence(
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if not record:
                    raise ValueError(
                        f"No result returned for decision evidence persistence: {decision_id}"
                    )

                # Emit metrics for evidence retrieval success
                for evidence in evidence_list:
                    ro_evidence_retrieval_success_total.labels(
            ...
            for evidence in evidence_list:
                ro_evidence_retrieval_failure_total.labels(
                    source_type=evidence.source_type
                ).inc()
            raise
    ...
    def read_decision_graph(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if not records:
                    return {"decision": None, "claims": [], "evidence": []}
    ...
    def update_decision_confidence(
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if record:
                    logger.info(
                        f"Updated decision confidence for {decision_id}: {confidence}"
                    )
                    return True
                else:
                    logger.error(
                        f"Failed to update decision confidence for {decision_id}"
                    )
                    return False
        ...
        try:
            ...
            with self._get_session() as session:
                ...
                if record:
                    ...
                    logger.error(
                        f"Failed to persist verification for decision {decision_id}"
                    )
                    return False

        except Exception as e:
            logger.error(
                f"Error persisting verification for decision {decision_id}: {e}"
            )
            return False
