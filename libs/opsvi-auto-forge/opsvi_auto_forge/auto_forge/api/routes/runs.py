...
from opsvi_auto_forge.config.models import Run, RunResponse, TaskRecord
...
async def start_pipeline_execution(run_id: str, neo4j_client: Neo4jClient) -> None:
    ...
    try:
        ...
        logger.info(
            f"🔵 Step 7: Pipeline started successfully for run {run_id}, DAG ID: {context.dag_id}"
        )
        ...
        logger.info(
            f"🔵 Step 9: Pipeline execution completed for run {run_id}, result: {result}"
        )
        ...
        try:
            ...
            logger.error(
                f"🔴 Failed to update run status after background task failure: {update_error}"
            )
    ...
    try:
        with RUN_OPERATION_DURATION.labels(operation="create").time():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to create run: {run_error}"
                )

            # Submit pipeline execution using the correct pipeline task
            logger.info(f"🔍 About to submit pipeline task for run {run_id}")
            print(f"🔍 PRINT: About to submit pipeline task for run {run_id}")
            try:
                from opsvi_auto_forge.infrastructure.workers.celery_app import (
                    execute_pipeline,
                )
                ...
                logger.info(
                    f"🔍 Successfully submitted pipeline task {task.id} for run {run_id}"
                )
                print(
                    f"🔍 PRINT: Successfully submitted pipeline task {task.id} for run {run_id}"
                )
            except Exception as task_error:
                logger.error(
                    f"🔴 Failed to submit pipeline task for run {run_id}: {task_error}"
                )
                import traceback

                logger.error(
                    f"🔴 Pipeline task submission traceback: {traceback.format_exc()}"
                )
...
async def get_run_artifacts(
    ...
    try:
        with RUN_OPERATION_DURATION.labels(operation="get_artifacts").time():
            query = """
            MATCH (r:Run {id: $run_id})-[:HAS_TASK]->(t:Task)-[:PRODUCES]->(a:Artifact)
            RETURN a
...
async def get_pipeline_visualization(
    ...
    try:
        with RUN_OPERATION_DURATION.labels(
            operation="get_pipeline_visualization"
        ).time():
            # Get run information
            run_query = """
            MATCH (r:Run {id: $run_id})
            RETURN r
            """
            run_result = await neo4j_client.execute_query(
                run_query, {"run_id": str(run_id)}
            )
            ...
            ORDER BY t.created_at ASC
            """
            tasks_result = await neo4j_client.execute_query(
                tasks_query, {"run_id": str(run_id)}
            )
            ...
            return {
                ...
                "completed_stages": len(
                    [s for s in pipeline_stages if s["status"] == "completed"]
                ),
                "running_stages": len(
                    [s for s in pipeline_stages if s["status"] == "running"]
                ),
                "failed_stages": len(
                    [s for s in pipeline_stages if s["status"] == "failed"]
                ),
            }
        ...
        RUN_OPERATION_COUNT.labels(
            operation="get_pipeline_visualization", status="error"
        ).inc()
        logger.error(f"Failed to get pipeline visualization for run {run_id}: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get pipeline visualization: {e}"
        )
