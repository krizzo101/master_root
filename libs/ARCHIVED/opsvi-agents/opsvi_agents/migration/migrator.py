...
from ..core import AgentConfig, BaseAgent
...
class AgentMigrator:
    ...
    def migrate_from_module(
        self, module_path: str, agent_class_name: str, new_name: str = None
    ) -> BaseAgent:
    ...
    def migrate_from_config(
        self, config_path: str, adapter_class: Type[BaseAgent] = UniversalAgentAdapter
    ) -> List[BaseAgent]:
        ...
        for config_data in configs:
            try:
                ...
                self.migration_report.append(
                    {"source": config_path, "name": config.name, "status": "success"}
                )
                ...
                for name, obj in vars(module).items():
                    if (
                        ...
                        agent = self.migrate_from_module(
                            str(file_path), name, f"{file_path.stem}_{name.lower()}"
                        )
                        agents.append(agent)
    ...
    def generate_report(self, output_path: str = None) -> Dict[str, Any]:
        ...
        report = {
            ...
            "failed": len(failed),
            "success_rate": len(successful) / len(self.migration_report)
