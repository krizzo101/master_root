...
class TransformAgent(BaseAgent):
    ...
    def _filter_data(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            if isinstance(filtered, (list, dict))
            else (1 if filtered else 0),
        }
    ...
    def _create_pipeline(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"pipeline_id": pipeline_id, "name": name, "rules_count": len(rules)}

    def _apply_custom_transform(
        self, data: Any, transform_type: str, parameters: Dict[str, Any]
    ) -> Any:
        ...
        if transform_type == "extract":
            ...
            if isinstance(data, str):
                return data.replace(
                    parameters.get("find", ""), parameters.get("replace", "")
                )
    ...
    def _apply_aggregations(
        self, data: List[Any], aggregations: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        for field, func in aggregations.items():
            if func == "count":
                ...
                values = [
                    item.get(field, 0) if isinstance(item, dict) else item
                    for item in data
                ]
                ...
                values = [
                    item.get(field) if isinstance(item, dict) else item for item in data
                ]
                ...
                result[field] = (
                    ...
                    else (data[0] if data else None)
                )
            elif func == "last":
                result[field] = (
                    data[-1].get(field)
                    if data and isinstance(data[-1], dict)
                    else (data[-1] if data else None)
                )
    ...
    def _create_filter_operation(self, parameters: Dict[str, Any]) -> Callable:
        ...
        def filter_op(data: Any, **kwargs) -> Any:
            if isinstance(data, list):
                return [
                    item for item in data if self._evaluate_condition(item, parameters)
                ]
            return data
    ...
    def _create_aggregate_operation(self, parameters: Dict[str, Any]) -> Callable:
        ...
        def aggregate_op(data: Any, **kwargs) -> Any:
            if isinstance(data, list):
                return self._apply_aggregations(
                    data, parameters.get("aggregations", {})
                )
            return data
    ...
    def shutdown(self) -> bool:
        ...
        logger.info("transform_agent_shutdown", pipelines_count=len(self.pipelines))
