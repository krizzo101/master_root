...
class MonitorAgent(BaseAgent):
    ...
    def monitor(
        self, target: str, metrics: List[MetricType], duration: int = 60
    ) -> MonitoringReport:
        """Monitor a target."""
        result = self.execute(
            {
    ...
    def _monitor_system(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for metric_name in metrics_list:
            metric_type = (
                MetricType[metric_name.upper()]
                if metric_name.upper() in MetricType.__members__
                else MetricType.CUSTOM
            )
            ...
            metric = Metric(
                name=f"{target}_{metric_name}", type=metric_type, value=value
            )
        ...
        return {"report": report, "report_id": report.id, "summary": report.summary}
    ...
    def _track_metric(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        type_enum = (
            MetricType[metric_type.upper()]
            if metric_type.upper() in MetricType.__members__
            else MetricType.CUSTOM
        )

        metric = Metric(name=name, type=type_enum, value=float(value), tags=tags)
        ...
        return {"metric": metric.to_dict(), "alert": alert.to_dict() if alert else None}
            ...
            if metric_history:
                ...
                if alert:
                    ...
                    for alert_id, existing_alert in self.alerts.items():
                        if (
                            existing_alert.metric
                            and existing_alert.metric.name == metric_name
    ...
    def _perform_health_check(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"health_check": health.to_dict(), "healthy": all_healthy}
    ...
    def _set_threshold(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"metric": metric_name, "threshold": threshold, "set": True}
    ...
    def _get_metrics(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if metric_name and metric_name in self.metrics:
            ...
            return {"metrics": [m.to_dict() for m in metrics], "statistics": stats}
        else:
    ...
    def _generate_report(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"report": report.to_dict(), "report_id": report.id}
    ...
    def _clear_alerts(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"cleared": cleared, "remaining": len(self.alerts)}
