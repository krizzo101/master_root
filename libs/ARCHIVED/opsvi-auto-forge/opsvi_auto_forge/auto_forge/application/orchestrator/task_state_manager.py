...
class TaskStateManager:
    ...
    async def update_state(self, task_id: UUID, status: TaskStatus, **metadata) -> None:
    ...
    async def cleanup_completed_tasks(self, max_age_hours: int = 24) -> int:
        ...
        try:
            ...
            result = await self.neo4j_client.run_query(
                query, parameters={"cutoff_time": cutoff_time.isoformat()}
            )
            ...
            logger.info(
                f"Cleaned up {cleaned_count} completed tasks older than {max_age_hours} hours"
            )
    ...
    async def _persist_state_to_neo4j(
        self, task_id: UUID, status: TaskStatus, metadata: Dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            await self.neo4j_client.run_query(
                query, parameters={"id": str(task_id), "data": neo4j_data}
            )
    ...
    async def _update_redis_state(
        self, task_id: UUID, state_data: Dict[str, Any]
    ) -> None:
        ...
        try:
            if self.redis_client:
                ...
                await self.redis_client.set(
                    key, json.dumps(state_data), ex=3600  # Expire after 1 hour
                )
        except Exception as e:
    ...
    async def _delete_from_neo4j(self, task_id: str) -> None:
        ...
        try:
            ...
            await self.neo4j_client.run_query(query, parameters={"task_id": task_id})
