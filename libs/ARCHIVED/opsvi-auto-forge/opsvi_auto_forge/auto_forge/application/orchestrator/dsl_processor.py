...
class ReasoningConfig(BaseModel):
    ...
    max_thoughts: int = Field(
        20, ge=1, le=100, description="Maximum thoughts per reasoning"
    )
    verifier: str = Field("gpt-4.1", description="Model to use for verification")
    min_confidence: float = Field(
        0.85, ge=0.0, le=1.0, description="Minimum confidence threshold"
    )
...
class KnowledgeConfig(BaseModel):
    ...
    freshness_days: int = Field(
        90, ge=1, le=365, description="Freshness window in days"
    )
    max_ctx_chars: int = Field(
        120000, ge=1000, le=500000, description="Max context characters"
    )
